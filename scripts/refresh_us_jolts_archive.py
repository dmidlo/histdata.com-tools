"""Refresh the compact real-artifact BLS JOLTS manifest.
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional retained corpus containing jolts-index.html and "
...
def _snapshot(request: OfficialSourceRequestV1) -> OfficialRawSnapshotV1:
    ...
    if response is None:
        raise RuntimeError(f"official JOLTS request exhausted: {request.uri}")
    response.raise_for_status()
    if not response.content:
        raise ValueError(f"official JOLTS response is empty: {request.uri}")
...
def _retained_snapshot(
    ...
    filename = (
        "jolts-index.html"
        if request.uri.endswith("/bls/news-release/jolts.htm")
        else Path(urlparse(request.uri).path).name
    )
    path = root / filename
    if not path.is_file():
        raise ValueError(f"retained JOLTS artifact is unavailable: {filename}")
    content = path.read_bytes()
    if not content:
        raise ValueError(f"retained JOLTS artifact is empty: {filename}")
    content_type = {
...
def _prefer_retained_index_evidence(
    ...
    observed_index: BlsJoltsReleaseIndexV1,
) -> tuple[OfficialRawSnapshotV1, BlsJoltsReleaseIndexV1]:
    """Avoid presentation-only index churn when inventory is unchanged."""
    manifest_path = asset_directory / "us_jolts_archive_v1.json"
    index_path = asset_directory / "us_jolts_release_index_v1.html.b64"
    if not manifest_path.exists() and not index_path.exists():
        return observed_snapshot, observed_index
    if not manifest_path.is_file() or not index_path.is_file():
        raise ValueError("retained JOLTS index evidence is incomplete")
    try:
        manifest = BlsJoltsArchiveManifestV1.from_json(
        ...
        raise ValueError("retained JOLTS index evidence is invalid") from exc
    ...
    if (
        ...
        raise ValueError("retained JOLTS index differs from the manifest")
    ...
    retained_as_observed = parse_bls_jolts_release_index(
...
def main(argv: list[str] | None = None) -> int:
    ...
    index_request = build_bls_jolts_index_request(registry, as_of_date=args.as_of)
    snapshot_loader = (
        _snapshot
    ...
    observed_index = parse_bls_jolts_release_index(
    ...
    requests_to_fetch = build_bls_jolts_archive_requests(registry, release_index)
        ...
        json.dumps(
            {
                ...
                "job_openings_revision_count": (manifest.job_openings_revision_count),
                "hires_revision_count": (manifest.hires_revision_count),
                "total_separations_revision_count": manifest.total_separations_revision_count,
                "noncomparable_revision_count": (manifest.noncomparable_revision_count),
                "annual_benchmark_release_count": (
                    manifest.annual_benchmark_release_count
                ),
