#!/usr/bin/env python3
"""Refresh or replay the official INSEE consumer-confidence archive for #539."""
...
from histdatacom.market_context.insee_consumer_confidence_archive import (
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 INSEE consumer-confidence qualification "
...
DEFAULT_SOURCE_DIRECTORY: Final = (
    REPO_ROOT
    / ".histdatacom"
    / "issue-539-insee-consumer-confidence"
    / "corpus"
)
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--asset-path",
        type=Path,
        default=packaged_insee_consumer_confidence_manifest_path(),
    )
    parser.add_argument(
        "--fetch-missing",
        action="store_true",
...
def _corpus_path(
    ...
    *,
    ...
    if role == "series":
        return root / "current" / "001587668.xml"
    raise ValueError(
        f"unsupported INSEE consumer confidence corpus role: {role}"
    )
...
def _validate_content(
    ...
    if request.source_format is OfficialSourceFormat.JSON:
        ...
        raise ValueError(
            f"retained INSEE consumer confidence SDMX differs: {source}"
        )
...
def _fetch(request: OfficialSourceRequestV1) -> bytes:
    ...
    for attempt in range(1, 7):
        ...
        if (
            ...
            raise ValueError(
                f"INSEE consumer confidence response resolved outside its host: {response.url}"
            )
        ...
        if reported.casefold() != expected:
            raise ValueError(
                "INSEE consumer confidence response content type differs: "
                f"expected {expected}, received {reported or '<missing>'}"
            )
        _validate_content(request, content, request.uri)
        return content
    raise RuntimeError(
        f"INSEE consumer confidence request retry bound exhausted: {request.uri}"
    )
    ...
    if not path.is_file():
        if not fetch_missing:
            raise FileNotFoundError(
                f"retained INSEE consumer confidence artifact is missing: {path}"
            )
...
def main() -> int:
    ...
    search_request = build_insee_consumer_confidence_search_request(registry)
    ...
    release_requests = build_insee_consumer_confidence_release_requests(
        registry, documents
    )
    release_snapshots = {
    ...
    catalog_snapshot = _snapshot(
        root,
        build_insee_consumer_confidence_catalog_request(registry),
        role="catalog",
        fetch_missing=args.fetch_missing,
    )
    series_snapshot = _snapshot(
        root,
        build_insee_consumer_confidence_current_series_request(registry),
        role="series",
        fetch_missing=args.fetch_missing,
    )
    manifest = build_insee_consumer_confidence_archive_manifest(
        registry,
        search,
        documents,
        release_snapshots,
