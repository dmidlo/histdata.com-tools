#!/usr/bin/env python3
"""Refresh or replay the official Eurostat construction output release archive."""
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 Eurostat construction output qualification "
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--as-of",
        default=EUROSTAT_CONSTRUCTION_OUTPUT_AS_OF_DATE,
        choices=(EUROSTAT_CONSTRUCTION_OUTPUT_AS_OF_DATE,),
        help="Frozen packaged qualification boundary.",
    )
    parser.add_argument(
        "--page-count",
        type=int,
        default=EUROSTAT_CONSTRUCTION_OUTPUT_SEARCH_PAGE_COUNT,
        choices=(EUROSTAT_CONSTRUCTION_OUTPUT_SEARCH_PAGE_COUNT,),
...
def _local_path(
    ...
    if request.source_format is OfficialSourceFormat.JSON_STAT:
        return source_directory / "datasets" / "sts_copr_m.json"
...
def _summary(
    manifest: EurostatConstructionOutputArchiveManifestV1,
) -> dict[str, object]:
    return {
...
def main() -> int:
    ...
    search_snapshots = tuple(
        capture(request)
        for request in build_eurostat_construction_output_search_requests(
            registry, page_count=args.page_count
        )
    )
    dataset_snapshot = capture(
        build_eurostat_construction_output_dataset_request(registry)
    )
    ...
    release_snapshots = tuple(
        capture(request)
        for request in build_eurostat_construction_output_release_requests(
            registry, inventory
        )
    )
    document_snapshots = tuple(
        capture(request)
        for request in build_eurostat_construction_output_document_requests(
            registry, inventory, release_snapshots
        )
    )
    manifest = build_eurostat_construction_output_archive_manifest(
        registry,
        search_snapshots,
        dataset_snapshot,
        release_snapshots,
    ...
    manifest_path = (
        asset_directory
        / packaged_eurostat_construction_output_manifest_path().name
    )
