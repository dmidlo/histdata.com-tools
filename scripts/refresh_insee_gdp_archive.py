#!/usr/bin/env python3
"""Refresh or replay the official INSEE quarterly GDP archive for issue #539."""
...
from histdatacom.market_context.insee_gdp_archive import (
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 INSEE quarterly GDP qualification "
    "(contact: repository maintainers)"
)
REPO_ROOT: Final = Path(__file__).resolve().parents[1]
DEFAULT_SOURCE_DIRECTORY: Final = (
    REPO_ROOT / ".histdatacom" / "issue-539-insee-gdp" / "corpus"
)
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--asset-path",
        type=Path,
        default=packaged_insee_gdp_manifest_path(),
    )
    parser.add_argument(
        "--fetch-missing",
        action="store_true",
...
def _corpus_path(
    ...
    *,
    ...
    if role == "series":
        return root / "current" / "CNT-2020-PIB-EQB-RF.xml"
    raise ValueError(f"unsupported INSEE GDP corpus role: {role}")
...
def _fetch(request: OfficialSourceRequestV1) -> bytes:
    ...
    for attempt in range(1, 7):
        ...
        if (
            ...
            raise ValueError(
                f"INSEE GDP response resolved outside its host: {response.url}"
            )
    ...
    if not path.is_file():
        if not fetch_missing:
            raise FileNotFoundError(
                f"retained INSEE GDP artifact is missing: {path}"
            )
...
def main() -> int:
    ...
    search_request = build_insee_gdp_search_request(registry)
    ...
    release_requests = build_insee_gdp_release_requests(registry, documents)
    release_snapshots = {
    ...
    series_request = build_insee_gdp_current_series_request(registry)
    ...
    manifest = build_insee_gdp_archive_manifest(
        registry,
        search,
        documents,
        release_snapshots,
