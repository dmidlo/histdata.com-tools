#!/usr/bin/env python3
"""Refresh or replay the complete official ECB monthly M3 archive."""
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 ECB monetary developments qualification "
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Caller-retained exact FOEDB and release-document corpus.",
    )
    parser.add_argument(
...
def _local_path(
    ...
    return source_directory / "monetary-developments" / name
...
def main() -> int:
    ...
    report_requests = build_ecb_monetary_developments_requests(
        registry, release_index
    )
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        report_snapshots = tuple(pool.map(capture, report_requests))
    manifest = build_ecb_monetary_developments_archive_manifest(
        registry,
        release_index,
    ...
    manifest_path = asset_directory / (
        packaged_ecb_monetary_developments_manifest_path().name
    )
        ...
        json.dumps(
            {
                ...
                "type_record_count": manifest.release_index.type_record_count,
                "pre_window_excluded_count": (
                    manifest.release_index.pre_window_excluded_count
                ),
                "post_boundary_excluded_count": (
                    manifest.release_index.post_boundary_excluded_count
                ),
                ...
                "adjustment_disclosure_count": (
                    manifest.adjustment_disclosure_count
                ),
                ...
                "release_date_correction_count": (
                    manifest.release_date_correction_count
                ),
