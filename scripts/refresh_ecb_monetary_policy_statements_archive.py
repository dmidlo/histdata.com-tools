#!/usr/bin/env python3
"""Refresh or replay the complete official ECB policy-statement archive."""
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 ECB monetary-policy statements "
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Caller-retained exact FOEDB and statement-page corpus.",
    )
    parser.add_argument(
        "--asset-directory",
        type=Path,
...
def main() -> int:
    ...
    release_index = build_ecb_foedb_statement_release_index(
        versions_snapshot,
        metadata_snapshot,
        chunk_snapshots,
        decision_archive,
    ...
    report_requests = build_ecb_statement_requests(registry, release_index)
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        report_snapshots = tuple(pool.map(capture, report_requests))
    manifest = build_ecb_monetary_policy_statements_archive_manifest(
        registry,
        release_index,
    ...
    manifest_path = asset_directory / (
        packaged_ecb_monetary_policy_statements_manifest_path().name
    )
        ...
        json.dumps(
            {
                ...
                "statement_count": len(manifest.statements),
                "linked_decision_count": manifest.linked_decision_count,
                "decision_publication_count": (manifest.decision_publication_count),
                "decision_without_statement_count": (
                    manifest.decision_without_statement_count
                ),
