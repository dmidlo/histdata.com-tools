"""Refresh the complete real-artifact BEA GDP archive manifest.
...
the HTML/PDF release corpus remains in an optional operator-owned directory.
"""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "release-YYYY-MM-DD-slug.html, and the two supporting PDFs."
        ),
    )
    parser.add_argument(
        "--workers",
...
def _snapshot(request: OfficialSourceRequestV1) -> OfficialRawSnapshotV1:
    ...
    if response is None:
        raise RuntimeError(f"official BEA GDP request exhausted: {request.uri}")
    response.raise_for_status()
    if not response.content:
        raise ValueError(f"official BEA GDP response is empty: {request.uri}")
...
def _retained_filename(request: OfficialSourceRequestV1) -> str:
    ...
    if request.window_start is None:
        raise ValueError("retained GDP release request has no date")
    return (
        f"release-{request.window_start}-"
        f"{Path(urlparse(request.uri).path).name}.html"
    )
...
def _retained_snapshot(
    ...
    if not path.is_file():
        raise ValueError(
            f"retained BEA GDP artifact is unavailable: {path.name}"
        )
    content = path.read_bytes()
    if not content:
        raise ValueError(f"retained BEA GDP artifact is empty: {path.name}")
    content_type = {
...
def _prefer_retained_index_evidence(
    ...
    observed_index: BeaGdpReleaseIndexV1,
) -> tuple[tuple[OfficialRawSnapshotV1, ...], BeaGdpReleaseIndexV1]:
    path = (
        asset_directory.expanduser().resolve() / "us_gdp_release_index_v1.json"
    )
    if not path.is_file():
    ...
    try:
        ...
        retained_index = parse_bea_gdp_release_index(
...
def _index_envelope(
    ...
    return {
        "schema_version": BEA_GDP_INDEX_ENVELOPE_SCHEMA_VERSION,
...
def main(argv: list[str] | None = None) -> int:
    ...
    index_requests = build_bea_gdp_index_requests(registry)
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        observed_index_snapshots = tuple(pool.map(loader, index_requests))
    observed_index = parse_bea_gdp_release_index(
    ...
    manifest_path = asset_directory / "us_gdp_archive_v1.json"
        ...
        json.dumps(
            {
                ...
                "advance_release_count": manifest.advance_release_count,
                "second_release_count": manifest.second_release_count,
                "final_release_count": manifest.final_release_count,
                "nonzero_revision_count": manifest.nonzero_revision_count,
                "advance_revision_count": manifest.advance_revision_count,
                "second_revision_count": manifest.second_revision_count,
                "final_revision_count": manifest.final_revision_count,
                "historical_update_notice_count": (
                    manifest.historical_update_notice_count
                ),
                "correction_notice_count": manifest.correction_notice_count,
                "shutdown_replacement_count": (
                    manifest.shutdown_replacement_count
                ),
                "shutdown_disruption_quarter_count": (
                    manifest.shutdown_disruption_quarter_count
                ),
                "fixed_width_release_count": (
                    manifest.fixed_width_release_count
                ),
                "narrative_release_count": manifest.narrative_release_count,
                "comparison_table_release_count": (
                    manifest.comparison_table_release_count
                ),
