#!/usr/bin/env python3
"""Refresh or replay the complete official ECB decision archive."""
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 ECB monetary-policy archive "
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Caller-retained exact FOEDB and decision-page corpus.",
    )
    parser.add_argument(
        "--asset-directory",
        type=Path,
...
def _read_or_fetch(
    ...
    if path.exists():
        if not path.is_file():
            raise ValueError(f"retained ECB path is not a file: {path}")
...
def main() -> int:
    ...
    release_index = build_ecb_foedb_release_index(
        versions_snapshot,
        metadata_snapshot,
        chunk_snapshots,
        as_of_date=args.as_of,
    )
    report_requests = build_ecb_decision_requests(registry, release_index)
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        report_snapshots = tuple(pool.map(capture, report_requests))
    manifest = build_ecb_monetary_policy_archive_manifest(
        registry,
        release_index,
    ...
    manifest_path = (
        asset_directory / packaged_ecb_monetary_policy_manifest_path().name
    )
        ...
        json.dumps(
            {
                ...
                "emergency_count": manifest.emergency_count,
