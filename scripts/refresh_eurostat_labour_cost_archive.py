#!/usr/bin/env python3
"""Refresh or replay the official Eurostat labour cost release archive."""
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 Eurostat labour cost qualification "
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--as-of",
        default=EUROSTAT_LABOUR_COST_AS_OF_DATE,
        choices=(EUROSTAT_LABOUR_COST_AS_OF_DATE,),
        help="Frozen packaged qualification boundary.",
    )
    parser.add_argument(
        "--page-count",
        type=int,
        default=EUROSTAT_LABOUR_COST_SEARCH_PAGE_COUNT,
        choices=(EUROSTAT_LABOUR_COST_SEARCH_PAGE_COUNT,),
...
def _local_path(source_directory: Path, request: OfficialSourceRequestV1) -> Path:
    ...
    if request.source_format is OfficialSourceFormat.JSON_STAT:
        return source_directory / "datasets" / "lc_lci_r2_q.json"
...
def _summary(
    manifest: EurostatLabourCostArchiveManifestV1,
) -> dict[str, object]:
    return {
        ...
        "current_dataset_comparison_count": (manifest.current_dataset_comparison_count),
        ...
        "reference_period_inferred_count": (manifest.reference_period_inferred_count),
        "composition_source_stated_count": (manifest.composition_source_stated_count),
...
def main() -> int:
    ...
    search_snapshots = tuple(
        capture(request)
        for request in build_eurostat_labour_cost_search_requests(
            registry, page_count=args.page_count
        )
    )
    dataset_snapshot = capture(build_eurostat_labour_cost_dataset_request(registry))
    inventory = parse_eurostat_labour_cost_inventory(
        search_snapshots, as_of_date=args.as_of
    )
    release_snapshots = tuple(
        capture(request)
        for request in build_eurostat_labour_cost_release_requests(registry, inventory)
    )
    document_snapshots = tuple(
        capture(request)
        for request in build_eurostat_labour_cost_document_requests(
            registry, inventory, release_snapshots
        )
    )
    manifest = build_eurostat_labour_cost_archive_manifest(
        registry,
        search_snapshots,
        dataset_snapshot,
        release_snapshots,
    ...
    manifest_path = asset_directory / packaged_eurostat_labour_cost_manifest_path().name
