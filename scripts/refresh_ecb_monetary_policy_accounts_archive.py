#!/usr/bin/env python3
"""Refresh or replay the complete official ECB meeting-account archive."""
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 ECB monetary-policy accounts "
    "qualification (contact: repository maintainers)"
)
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Caller-retained exact FOEDB and account-page corpus.",
    )
    parser.add_argument(
        "--asset-directory",
        type=Path,
...
def _local_path(source_directory: Path, request: OfficialSourceRequestV1) -> Path:
    ...
    return source_directory / "reports" / name
...
def main() -> int:
    ...
    decision_archive = load_packaged_ecb_monetary_policy_archive_manifest()
    ...
    release_index = build_ecb_foedb_account_release_index(
        versions_snapshot,
        metadata_snapshot,
        chunk_snapshots,
        as_of_date=args.as_of,
    )
    report_requests = build_ecb_account_requests(registry, release_index)
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        report_snapshots = tuple(pool.map(capture, report_requests))
    manifest = build_ecb_monetary_policy_accounts_archive_manifest(
        registry,
        release_index,
    ...
    manifest_path = (
        asset_directory / packaged_ecb_monetary_policy_accounts_manifest_path().name
    )
        ...
        json.dumps(
            {
                ...
                "decision_archive_manifest_id": (manifest.decision_archive_manifest_id),
                ...
                "database_chunk_count": sum(
                    item.role.value == "foedb-chunk"
                    for item in manifest.release_index.database_artifacts
                ),
                "account_count": len(manifest.accounts),
                "linked_decision_count": manifest.linked_decision_count,
                ...
                "inferred_meeting_year_count": (manifest.inferred_meeting_year_count),
                "minimum_release_lag_days": (manifest.minimum_release_lag_days),
                "maximum_release_lag_days": (manifest.maximum_release_lag_days),
