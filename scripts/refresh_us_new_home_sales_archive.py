#!/usr/bin/env python3
"""Refresh or replay the complete Census New Residential Sales corpus."""
...
USER_AGENT = (
    "histdata.com-tools new-home-sales archive qualification "
...
def main() -> None:
    ...
    index_request = build_census_new_home_sales_index_request(
    ...
    for request in build_census_new_home_sales_archive_requests(
    ...
    manifest = build_census_new_home_sales_archive_manifest(
    ...
    asset_directory = (
        ...
        else packaged_census_new_home_sales_archive_manifest_path().parent
    )
    asset_directory.mkdir(parents=True, exist_ok=True)
    index_payload = {
        "schema_version": NEW_HOME_SALES_INDEX_ENVELOPE_SCHEMA_VERSION,
        "snapshot": index_snapshot.to_dict(),
        "content_base64": base64.b64encode(index_content).decode("ascii"),
    }
    (asset_directory / packaged_census_new_home_sales_index_path().name).write_text(
        canonical_contract_json(index_payload) + "\n", encoding="utf-8"
    )
    (
        asset_directory / packaged_census_new_home_sales_archive_manifest_path().name
