#!/usr/bin/env python3
"""Refresh or replay the official INSEE CPI archive for issue #539."""
...
from histdatacom.market_context.insee_cpi_archive import (
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 INSEE CPI qualification "
    "(contact: repository maintainers)"
)
REPO_ROOT: Final = Path(__file__).resolve().parents[1]
DEFAULT_SOURCE_DIRECTORY: Final = (
    REPO_ROOT / ".histdatacom" / "issue-539-insee-cpi" / "corpus"
)
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--asset-path",
        type=Path,
        default=packaged_insee_cpi_manifest_path(),
    )
    parser.add_argument(
        "--fetch-missing",
        action="store_true",
...
def _corpus_path(
    ...
    *,
    ...
    if role == "cpi-catalog":
        return root / "current" / "cpi-series.json"
    if role == "hicp-catalog":
        return root / "current" / "hicp-series.json"
    if role == "release":
        return (
            root
            / "releases"
    ...
    raise ValueError(f"unsupported INSEE corpus role: {role}")
...
def _fetch(request: OfficialSourceRequestV1) -> bytes:
    ...
    for attempt in range(1, 7):
        ...
        if (
            ...
            raise ValueError(
                f"INSEE response resolved outside its host: {response.url}"
            )
        ...
        if reported.casefold() != expected:
            raise ValueError(
                "INSEE response content type differs: "
                f"expected {expected}, received {reported or '<missing>'}"
            )
        _validate_content(request, content, request.uri)
        return content
    raise RuntimeError(f"INSEE request retry bound exhausted: {request.uri}")
    ...
    if not path.is_file():
        if not fetch_missing:
            raise FileNotFoundError(
                f"retained INSEE artifact is missing: {path}"
            )
...
def main() -> int:
    ...
    search_request = build_insee_cpi_search_request(registry)
    ...
    release_requests = build_insee_cpi_release_requests(registry, documents)
    release_snapshots = {
    ...
    catalog_requests = build_insee_cpi_catalog_requests(registry)
    catalog_snapshots = tuple(
        ...
        for request, role in zip(
            catalog_requests,
            ("cpi-catalog", "hicp-catalog"),
            strict=True,
        )
    )
    series_requests = build_insee_cpi_current_series_requests(registry)
    current_series_snapshots = {
        urlsplit(request.uri).path.rsplit("/", 1)[-1]: _snapshot(
            root,
            request,
            role="series",
            fetch_missing=args.fetch_missing,
    ...
    manifest = build_insee_cpi_archive_manifest(
        registry,
        search,
        documents,
        release_snapshots,
    ...
    print(
        ...
        f"{manifest.released_value_count} values, "
        f"{len(manifest.revisions)} comparable revisions)"
    )
    return 0
