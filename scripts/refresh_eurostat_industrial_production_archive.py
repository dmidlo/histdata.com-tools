#!/usr/bin/env python3
"""Refresh or replay the official Eurostat industrial production release archive."""
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 Eurostat industrial production qualification "
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--as-of",
        default=EUROSTAT_INDUSTRIAL_PRODUCTION_AS_OF_DATE,
        choices=(EUROSTAT_INDUSTRIAL_PRODUCTION_AS_OF_DATE,),
        help="Frozen packaged qualification boundary.",
    )
    parser.add_argument(
        "--page-count",
        type=int,
        default=EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_PAGE_COUNT,
        choices=(EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_PAGE_COUNT,),
...
def _local_path(
    ...
    if request.source_format is OfficialSourceFormat.JSON_STAT:
        return source_directory / "datasets" / "sts_inpr_m.json"
        ...
        for attempt in range(1, MAX_FETCH_ATTEMPTS + 1):
            ...
            try:
                ...
                if request.source_format in {
                    ...
                    _product_code(request.uri) not in parsed.path.casefold()
                ):
                    raise RuntimeError(
...
def _summary(
    manifest: EurostatIndustrialProductionArchiveManifestV1,
) -> dict[str, object]:
    return {
...
def main() -> int:
    ...
    search_snapshots = tuple(
        capture(request)
        for request in build_eurostat_industrial_production_search_requests(
            registry, page_count=args.page_count
        )
    )
    dataset_snapshot = capture(
        build_eurostat_industrial_production_dataset_request(registry)
    )
    ...
    release_snapshots = tuple(
        capture(request)
        for request in build_eurostat_industrial_production_release_requests(
            registry, inventory
        )
    )
    document_snapshots = tuple(
        capture(request)
        for request in build_eurostat_industrial_production_document_requests(
            registry, inventory, release_snapshots
        )
    )
    manifest = build_eurostat_industrial_production_archive_manifest(
        registry,
        search_snapshots,
        dataset_snapshot,
        release_snapshots,
    ...
    manifest_path = (
        asset_directory
        / packaged_eurostat_industrial_production_manifest_path().name
    )
