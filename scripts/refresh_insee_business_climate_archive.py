#!/usr/bin/env python3
"""Refresh or replay the official INSEE business-climate archive for #539."""
...
from histdatacom.market_context.insee_business_climate_archive import (
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 INSEE business-climate qualification "
    "(contact: repository maintainers)"
)
REPO_ROOT: Final = Path(__file__).resolve().parents[1]
DEFAULT_SOURCE_DIRECTORY: Final = (
    REPO_ROOT / ".histdatacom" / "issue-539-insee-business-climate" / "corpus"
)
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--asset-path",
        type=Path,
        default=packaged_insee_business_climate_manifest_path(),
    )
    parser.add_argument(
        "--fetch-missing",
        action="store_true",
...
def _corpus_path(
    ...
    *,
    ...
    if role == "search":
        ...
        if query == '"Business climate indicator and turning point indicator"':
            return root / "search-legacy.json"
        if query == '"Business indicator"':
            return root / "search-current.json"
        raise ValueError(
            f"unsupported INSEE business climate search query: {query}"
        )
    if role == "release":
        return (
            root
    ...
    if role == "series":
        return root / "current" / "001565530.xml"
    raise ValueError(f"unsupported INSEE business climate corpus role: {role}")
...
def _validate_content(
    ...
    if request.source_format is OfficialSourceFormat.JSON:
        ...
        raise ValueError(
            f"retained INSEE business climate SDMX differs: {source}"
        )
...
def _fetch(request: OfficialSourceRequestV1) -> bytes:
    ...
    for attempt in range(1, 7):
        ...
        if (
            ...
            raise ValueError(
                f"INSEE business climate response resolved outside its host: {response.url}"
            )
        ...
        if reported.casefold() != expected:
            raise ValueError(
                "INSEE business climate response content type differs: "
                f"expected {expected}, received {reported or '<missing>'}"
            )
        _validate_content(request, content, request.uri)
        return content
    raise RuntimeError(
        f"INSEE business climate request retry bound exhausted: {request.uri}"
    )
    ...
    if not path.is_file():
        if not fetch_missing:
            raise FileNotFoundError(
                f"retained INSEE business climate artifact is missing: {path}"
            )
...
def main() -> int:
    ...
    search_snapshots = tuple(
        ...
        for request in build_insee_business_climate_search_requests(registry)
    )
    search, documents = parse_insee_business_climate_search(
        search_snapshots,
        as_of_date=args.as_of,
    )
    release_requests = build_insee_business_climate_release_requests(
        registry, documents
    )
    release_snapshots = {
    ...
    catalog_snapshot = _snapshot(
        root,
        build_insee_business_climate_catalog_request(registry),
        role="catalog",
        fetch_missing=args.fetch_missing,
    )
    series_snapshot = _snapshot(
        root,
        build_insee_business_climate_current_series_request(registry),
        role="series",
        fetch_missing=args.fetch_missing,
    )
    manifest = build_insee_business_climate_archive_manifest(
        registry,
        search,
        documents,
        release_snapshots,
