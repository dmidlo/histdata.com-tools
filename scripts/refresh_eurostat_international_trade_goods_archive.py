#!/usr/bin/env python3
"""Refresh or replay the official Eurostat international trade in goods release archive."""
...
USER_AGENT: Final = (
    "histdata.com-tools issue-539 Eurostat international-trade-in-goods qualification "
...
_typed_archive_product_code: Callable[[object], str] = _archive_product_code
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--as-of",
        default=EUROSTAT_INTERNATIONAL_TRADE_GOODS_AS_OF_DATE,
        choices=(EUROSTAT_INTERNATIONAL_TRADE_GOODS_AS_OF_DATE,),
        help="Frozen packaged qualification boundary.",
    )
    parser.add_argument(
        "--page-count",
        type=int,
        default=EUROSTAT_INTERNATIONAL_TRADE_GOODS_SEARCH_PAGE_COUNT,
        choices=(EUROSTAT_INTERNATIONAL_TRADE_GOODS_SEARCH_PAGE_COUNT,),
...
def _product_code(uri: str) -> str:
    return _typed_archive_product_code(uri)
...
def _local_path(
    ...
    if request.page_number is not None:
        return (
            source_directory / "index" / f"page_{request.page_number:03d}.xml"
        )
    if request.source_format is OfficialSourceFormat.JSON_STAT:
        return source_directory / "datasets" / "ext_st_easitc.json"
...
def _resolved_uri(request: OfficialSourceRequestV1, content: bytes) -> str:
    ...
    if (
        ...
        raise ValueError(
            "retained Eurostat landing has no unique canonical URI"
        )
...
class _PoliteFetcher:
    """Fetch sequentially with the empirically qualified request floor."""
    ...
    def fetch(self, request: OfficialSourceRequestV1) -> tuple[bytes, str, str]:
        ...
        for attempt in range(1, MAX_FETCH_ATTEMPTS + 1):
            ...
            try:
                ...
                if parsed.scheme != "https" or parsed.netloc != "ec.europa.eu":
                    raise RuntimeError(
                        "Eurostat response left the allowlisted host"
                    )
                if request.source_format in {
                    ...
                    try:
                        response_product_code = _product_code(response.url)
                    except ValueError as exc:
                        raise RuntimeError(
                            "Eurostat response has no qualified product identity"
                        ) from exc
                    if response_product_code != _product_code(request.uri):
                        raise RuntimeError(
    ...
    *,
    ...
    if path.exists():
        ...
        try:
            ...
            raise ValueError(
                f"retained Eurostat artifact differs: {path}"
            ) from exc
...
def _summary(
    manifest: EurostatInternationalTradeGoodsArchiveManifestV1,
) -> dict[str, object]:
    return {
        ...
        "current_dataset_comparison_count": (
            manifest.current_dataset_comparison_count
        ),
        "changed_current_dataset_count": manifest.changed_current_dataset_count,
        ...
        "reference_period_inferred_count": (
            manifest.reference_period_inferred_count
        ),
        "composition_source_stated_count": (
            manifest.composition_source_stated_count
        ),
...
def main() -> int:
    ...
    def capture(request: OfficialSourceRequestV1) -> OfficialRawSnapshotV1:
        ...
        return _snapshot(
            request, content, resolved_uri, content_type, captured_at_ns
        )

    search_snapshots = tuple(
        capture(request)
        for request in build_eurostat_international_trade_goods_search_requests(
            registry, page_count=args.page_count
        )
    )
    dataset_snapshot = capture(
        build_eurostat_international_trade_goods_dataset_request(registry)
    )
    inventory = parse_eurostat_international_trade_goods_inventory(
        search_snapshots, as_of_date=args.as_of
    )
    release_snapshots = tuple(
        capture(request)
        for request in build_eurostat_international_trade_goods_release_requests(
            registry, inventory
        )
    )
    document_snapshots = tuple(
        capture(request)
        for request in build_eurostat_international_trade_goods_document_requests(
            registry, inventory, release_snapshots
        )
    )
    manifest = build_eurostat_international_trade_goods_archive_manifest(
        registry,
        search_snapshots,
        dataset_snapshot,
        release_snapshots,
    ...
    manifest_path = (
        asset_directory
        / packaged_eurostat_international_trade_goods_manifest_path().name
    )
