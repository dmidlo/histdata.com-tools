"""Refresh the compact real-artifact BLS PPI manifest for issue #538.
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional retained corpus containing ppi-index.html and release "
...
def _snapshot(request: OfficialSourceRequestV1) -> OfficialRawSnapshotV1:
    ...
    if response is None:
        raise RuntimeError(f"official PPI request exhausted: {request.uri}")
    response.raise_for_status()
    if not response.content:
        raise ValueError(f"official PPI response is empty: {request.uri}")
...
def _retained_snapshot(
    ...
    filename = (
        "ppi-index.html"
        if request.uri.endswith("/bls/news-release/ppi.htm")
        else Path(urlparse(request.uri).path).name
    )
    path = root / filename
    if not path.is_file():
        raise ValueError(f"retained PPI artifact is unavailable: {filename}")
    content = path.read_bytes()
    if not content:
        raise ValueError(f"retained PPI artifact is empty: {filename}")
    content_type = {
...
def _prefer_retained_index_evidence(
    ...
    observed_index: BlsPpiReleaseIndexV1,
) -> tuple[OfficialRawSnapshotV1, BlsPpiReleaseIndexV1]:
    """Avoid presentation-only index churn when its inventory is unchanged."""
    manifest_path = asset_directory / "us_ppi_archive_v1.json"
    index_path = asset_directory / "us_ppi_release_index_v1.html.b64"
    if not manifest_path.exists() and not index_path.exists():
        return observed_snapshot, observed_index
    if not manifest_path.is_file() or not index_path.is_file():
        raise ValueError("retained PPI index evidence is incomplete")
    try:
        manifest = BlsPpiArchiveManifestV1.from_json(
        ...
        raise ValueError("retained PPI index evidence is invalid") from exc
    ...
    if (
        ...
        raise ValueError("retained PPI index bytes differ from the manifest")
    ...
    retained_as_observed = parse_bls_ppi_release_index(
...
def main(argv: list[str] | None = None) -> int:
    ...
    index_request = build_bls_ppi_index_request(registry, as_of_date=args.as_of)
    snapshot_loader = (
        _snapshot
    ...
    observed_index = parse_bls_ppi_release_index(
    ...
    requests_to_fetch = build_bls_ppi_archive_requests(registry, release_index)
