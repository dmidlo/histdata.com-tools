"""Tests for Temporal orchestration activity wrappers."""
...
def test_validate_urls_activity_returns_form_metadata(
    ...
    """URL validation should be callable as an orchestration activity."""
    monkeypatch.setattr(
        "histdatacom.activity_stages.fetch_histdata_page_data",
        lambda url, timeout: {
            "html": _form_html(),
...
def test_validate_urls_activity_persists_live_status_without_inspect(
    ...
    store = ManifestStatusStore(tmp_path / "orchestration-status")
    request = RunRequest(
        request_id="run-live-status",
        data_directory=str(tmp_path / "data"),
        metadata={
            STATUS_STORE_REF_KEY: store.status_store_ref(),
            "orchestration_task_queues": {"orchestration": "test-queue"},
        },
    )
    ...
    snapshot = OrchestrationJobSnapshot.from_dict(stored_snapshot)
...
def test_import_to_influx_activity_uses_request_influx_config(
    ...
    """Orchestration workers should use caller-resolved Influx connection config."""
    FakeInfluxWriter.instances.clear()
    FakeInfluxWriter.fail_with = None
    monkeypatch.setattr(
        "histdatacom.orchestration.activities._influx_batch_writer",
