"""Tests for the Eurostat unemployment archive refresh boundary."""
...
from scripts import refresh_eurostat_unemployment_archive as refresh
...
def _product_request() -> OfficialSourceRequestV1:
    ...
    return OfficialSourceRequestV1(
        ...
        uri=("https://ec.europa.eu/eurostat/product?code=3-01092026-bp"),
...
def _document_request(
    ...
    return OfficialSourceRequestV1(
        ...
        uri=(
            ...
            f"3-04012002-AP-EN.{suffix}/source"
        ),
...
def test_unemployment_refresh_prefers_equivalent_canonical_landing_uri() -> (
    None
):
    content = b"""<html><head>
    <link rel="canonical" href="https://ec.europa.eu/eurostat/web/products-euro-indicators/w/3-01092026-bp">
    <meta property="og:url" content="https://ec.europa.eu/eurostat/en/web/products-euro-indicators/w/3-01092026-bp">
    </head></html>"""
...
def test_unemployment_refresh_rejects_conflicting_landing_uri_metadata() -> (
    None
):
    content = b"""<html><head>
    <link rel="canonical" href="https://ec.europa.eu/eurostat/web/products-euro-indicators/w/3-01092026-bp">
    <meta property="og:url" content="https://ec.europa.eu/eurostat/en/web/products-euro-indicators/w/3-30072026-bp">
    </head></html>"""
...
def test_unemployment_refresh_retains_release_documents_separately(
    tmp_path: Path,
    source_format: OfficialSourceFormat,
    suffix: str,
) -> None:
    ...
    assert refresh._local_path(tmp_path, request) == (
        tmp_path / "documents" / f"3-04012002-ap.{suffix}"
    )
...
def test_unemployment_refresh_rejects_invalid_request_interval(
    value: float,
) -> None:
...
def test_unemployment_refresh_spaces_retry_after_connection_error(
