"""Tests for the bounded INSEE industrial production refresh boundary."""
...
from scripts import refresh_insee_industrial_production_archive as refresh
...
def test_refresh_maps_requests_to_stable_operator_corpus_paths(
    ...
    search = build_insee_industrial_production_search_requests(registry)[0]
    release = build_insee_industrial_production_release_requests(
        registry, ({"id": 9_050_846},)
    )[0]
    ...
    assert refresh._corpus_path(tmp_path, release, role="release") == (
        tmp_path / "releases/9050846.html"
    )
    assert refresh._corpus_path(tmp_path, catalogs[0], role="catalog") == (
        tmp_path / "current/catalog-0.json"
    )
    assert refresh._corpus_path(tmp_path, catalogs[1], role="catalog") == (
        tmp_path / "current/catalog-1000.json"
    )
    assert refresh._corpus_path(tmp_path, series[0], role="series") == (
        tmp_path / "current/010768261.xml"
    )
    assert refresh._corpus_path(tmp_path, series[1], role="series") == (
        tmp_path / "current/010768265.xml"
    )
    with pytest.raises(
        ValueError, match="unsupported INSEE industrial production corpus role"
    ):
...
def test_refresh_reads_retained_bytes_without_network(tmp_path: Path) -> None:
    request = build_insee_industrial_production_search_requests(
        load_packaged_official_source_registry()
    )[0]
...
def test_refresh_rejects_missing_or_mistyped_retained_artifacts(
    ...
    with pytest.raises(
        FileNotFoundError,
        match="retained INSEE industrial production artifact is missing",
    ):
        refresh._snapshot(
            tmp_path,
            request,
    ...
    with pytest.raises(
        ValueError, match="retained INSEE industrial production JSON differs"
    ):
        refresh._snapshot(
            tmp_path,
            request,
...
def test_refresh_rejects_same_host_redirect_before_relabeling_evidence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = build_insee_industrial_production_search_requests(
        load_packaged_official_source_registry()
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: _Response(
            "https://www.insee.fr/en/statistiques/9050846"
        ),
    )

    with pytest.raises(ValueError, match="resolved to a different endpoint"):
        refresh._fetch(request)
