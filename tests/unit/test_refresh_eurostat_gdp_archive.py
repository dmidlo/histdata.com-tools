"""Tests for the Eurostat GDP archive refresh boundary."""
...
from scripts import refresh_eurostat_gdp_archive as refresh
...
def _product_request() -> OfficialSourceRequestV1:
    ...
    return OfficialSourceRequestV1(
        ...
        uri=("https://ec.europa.eu/eurostat/product?" "code=2-08032024-ap"),
...
def _document_request(
    ...
    return OfficialSourceRequestV1(
        ...
        uri=(
            ...
            f"2-10012002-AP-EN.{suffix}/source"
        ),
...
def test_gdp_refresh_prefers_equivalent_canonical_landing_uri() -> None:
    content = b"""<html><head>
    <link rel="canonical" href="https://ec.europa.eu/eurostat/web/products-euro-indicators/w/2-08032024-ap">
    <meta property="og:url" content="https://ec.europa.eu/eurostat/en/web/products-euro-indicators/w/2-08032024-ap">
    </head></html>"""

    assert refresh._resolved_uri(_product_request(), content) == (
        "https://ec.europa.eu/eurostat/web/"
        "products-euro-indicators/w/2-08032024-ap"
    )


def test_gdp_refresh_rejects_conflicting_landing_uri_metadata() -> None:
    content = b"""<html><head>
    <link rel="canonical" href="https://ec.europa.eu/eurostat/web/products-euro-indicators/w/2-08032024-ap">
    <meta property="og:url" content="https://ec.europa.eu/eurostat/en/web/products-euro-indicators/w/2-07062024-ap">
    </head></html>"""
...
def test_gdp_refresh_retains_release_documents_separately(
    tmp_path: Path,
    source_format: OfficialSourceFormat,
    suffix: str,
) -> None:
    ...
    assert refresh._local_path(tmp_path, request) == (
        tmp_path / "documents" / f"2-10012002-ap.{suffix}"
    )
...
def test_gdp_refresh_rejects_invalid_request_interval(value: float) -> None:
    with pytest.raises(ValueError, match="between 13 and 3600 seconds"):
        refresh._PoliteFetcher(value)


def test_gdp_refresh_spaces_retry_after_connection_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = build_eurostat_gdp_dataset_request(
        load_packaged_official_source_registry()
    )
    clock = _Clock()
    starts: list[float] = []
