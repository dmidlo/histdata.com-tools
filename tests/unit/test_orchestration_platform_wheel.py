...
def _write_manifest(source_root: Path) -> None:
    ...
    asset_root = source_root / "src/histdatacom/orchestration/assets"
...
def test_prepare_runtime_binary_patches_manifest_and_copies_executable(
    ...
    bundled = (
        source_root
        / "src/histdatacom/orchestration/assets/bin/macos-arm64/temporal"
    )
    provenance_path = (
        source_root
        / "src/histdatacom/orchestration/assets/temporal-cli-provenance.json"
    )
    manifest = json.loads(
        (
            source_root / "src/histdatacom/orchestration/assets/manifest.json"
