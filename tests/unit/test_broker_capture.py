...
def test_live_source_surfaces_fixture_health_ordering_and_clock_drift() -> None:
    with generated_provider_scope(generated_legacy_request(_session(3))):
        session = _session(3)
        ...
        assert all(
            current.receive_time_monotonic_ns
            >= previous.receive_time_monotonic_ns
        ...
        gap = next(
            event
            for event in events
            if event.kind is BrokerCaptureEventKind.GAP
        )
...
def test_append_only_rotation_replay_and_live_consumer_parity(
    ...
    with generated_provider_scope(generated_legacy_request(_session(4))):
        session = _session(4)
        ...
        live_result = consume_broker_capture_source(
            ...
            provider_request=generated_legacy_request(source.session),
        )
        ...
        assert inspect_broker_capture_session(
        ...
        assert (
            BrokerCaptureSessionManifestV1.from_json(manifest.to_json())
            == manifest
        )
        assert discover_broker_capture_session_manifests(tmp_path) == (
            manifest,
        )
        assert (
...
def test_private_adapter_configuration_never_enters_capture_artifacts(
    ...
    with generated_provider_scope(generated_legacy_request(_session(5))):
...
def test_partial_and_orphan_artifacts_are_detected_but_not_advertised(
    ...
    with generated_provider_scope(generated_legacy_request(_session(6))):
        session = _session(6)
        source, _messages = _fixture_source(session)
        writer = AppendOnlyBrokerCaptureWriterV1(
            tmp_path,
...
def test_quota_backpressure_and_retention_refuse_predictably(
    ...
    with generated_provider_scope(
        generated_legacy_request(_session(7)),
        generated_legacy_request(_session(8)),
        generated_legacy_request(_session(9)),
    ):
        ...
        retention_writer = AppendOnlyBrokerCaptureWriterV1(
            ...
            provider_request=generated_legacy_request(retention_session),
        )
        retention_writer.append(retention_events[0])
        with pytest.raises(
            BrokerCaptureRetentionError, match="retention ceiling"
        ):
        ...
        # Paired policy receipts are real retained bytes, not a quota exemption.
        reserve = 1024 * 1024
        quota_writer = AppendOnlyBrokerCaptureWriterV1(
            ...
            provider_request=generated_legacy_request(quota_session),
        )
        ...
        pressure_line_size = len(
            (pressure_event.to_json() + "\n").encode("utf-8")
        )
        pressure_writer = AppendOnlyBrokerCaptureWriterV1(
            ...
            storage_policy=_policy(
                ...
                manifest_reserve_bytes=reserve,
                max_session_bytes=pressure_line_size + reserve + 2 * 1024**2,
                high_watermark_bytes=pressure_line_size + reserve,
            ),
            provider_request=generated_legacy_request(pressure_session),
        )
        with pytest.raises(
            BrokerCaptureBackpressureError, match="high watermark"
        ):
...
def test_existing_session_and_corrupt_replay_fail_closed(
    ...
    with generated_provider_scope(generated_legacy_request(_session(10))):
        session = _session(10)
        source, _messages = _fixture_source(session)
        policy = _policy()
        writer = AppendOnlyBrokerCaptureWriterV1(
