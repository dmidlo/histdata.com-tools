"""Tests for issue-180 orchestration throughput benchmark helpers."""
...
from histdatacom.orchestration.live_smoke import LiveOrchestrationStopError
from histdatacom.orchestration.performance import BenchmarkMeasurement
...
class _ThroughputSupervisor:
    ...
    def _status(self, state: str) -> OrchestrationStatus:
...
def test_throughput_report_serializes_performance_envelope(
    ...
    config = build_orchestration_worker_config(runtime_policy=runtime_policy)
...
def test_live_orchestration_throughput_benchmark_records_stopped_status(
    tmp_path: Path,
) -> None:
...
def test_live_orchestration_throughput_benchmark_fails_on_stopping_shutdown(
    tmp_path: Path,
    ...
    with pytest.raises(LiveOrchestrationStopError, match="state=stopping"):
        run_live_orchestration_throughput_benchmark(
            workspace=tmp_path / "workspace",
            runtime_home=tmp_path / "runtime",
            data_directory=tmp_path / "data",
