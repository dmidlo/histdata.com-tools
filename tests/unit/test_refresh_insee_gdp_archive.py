"""Tests for the bounded INSEE quarterly GDP refresh boundary."""
...
from scripts import refresh_insee_gdp_archive as refresh
...
def test_refresh_maps_requests_to_stable_operator_corpus_paths(
    ...
    release = build_insee_gdp_release_requests(registry, ({"id": 9033400},))[0]
    ...
    assert refresh._corpus_path(tmp_path, release, role="release") == (
        tmp_path / "releases/9033400.html"
    )
    assert refresh._corpus_path(tmp_path, series, role="series") == (
        tmp_path / "current/CNT-2020-PIB-EQB-RF.xml"
    )
    with pytest.raises(ValueError, match="unsupported INSEE GDP corpus role"):
...
def test_refresh_reads_retained_bytes_without_network(tmp_path: Path) -> None:
    registry = load_packaged_official_source_registry()
    request = build_insee_gdp_search_request(registry)
...
def test_refresh_rejects_missing_or_mistyped_retained_artifacts(
    ...
    with pytest.raises(
        FileNotFoundError, match="retained INSEE GDP artifact is missing"
    ):
        refresh._snapshot(
            tmp_path,
            request,
    ...
    with pytest.raises(ValueError, match="retained INSEE GDP JSON differs"):
        refresh._snapshot(
            tmp_path,
            request,
            role="search",
...
def test_refresh_retries_transient_status_with_identifying_agent(
    ...
    request = build_insee_gdp_search_request(
        load_packaged_official_source_registry()
    )
    responses = iter(
        (
