"""Tests for the bounded INSEE consumer confidence refresh boundary."""
...
from scripts import refresh_insee_consumer_confidence_archive as refresh
...
def test_refresh_maps_requests_to_stable_operator_corpus_paths(
    ...
    release = build_insee_consumer_confidence_release_requests(
        registry, ({"id": 9_038_311},)
    )[0]
    ...
    assert refresh._corpus_path(tmp_path, release, role="release") == (
        tmp_path / "releases/9038311.html"
    )
    ...
    assert refresh._corpus_path(tmp_path, series, role="series") == (
        tmp_path / "current/001587668.xml"
    )
    with pytest.raises(
        ValueError, match="unsupported INSEE consumer confidence corpus role"
    ):
...
def test_refresh_reads_retained_bytes_without_network(tmp_path: Path) -> None:
    request = build_insee_consumer_confidence_search_request(
        load_packaged_official_source_registry()
    )
...
def test_refresh_rejects_missing_or_mistyped_retained_artifacts(
    ...
    with pytest.raises(
        FileNotFoundError,
        match="retained INSEE consumer confidence artifact is missing",
    ):
        refresh._snapshot(
            tmp_path,
            request,
    ...
    with pytest.raises(
        ValueError, match="retained INSEE consumer confidence JSON differs"
    ):
        refresh._snapshot(
            tmp_path,
            request,
