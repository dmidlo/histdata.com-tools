...
from histdatacom.orchestration import live_smoke
...
def test_check_live_runtime_smoke_returns_live_report(
    ...
    def fake_run_live_orchestration_smoke(**kwargs: Any) -> _Result:
    ...
    monkeypatch.setattr(
        ...
        fake_run_live_orchestration_smoke,
    )
...
def test_check_hermetic_runtime_smoke_returns_hermetic_report(
    ...
    def fake_run_hermetic_orchestration_smoke(**kwargs: Any) -> _Result:
        captured.update(kwargs)
        return _Result()
    ...
    monkeypatch.setattr(
        ...
        fake_run_hermetic_orchestration_smoke,
    )
...
def test_check_default_routing_runtime_smoke_returns_report(
    ...
    def fake_run_default_client_routing_orchestration_smoke(
        **kwargs: Any,
    ) -> _Result:
        captured.update(kwargs)
        return _Result()
    ...
    monkeypatch.setattr(
        ...
        fake_run_default_client_routing_orchestration_smoke,
    )
...
def test_smoke_command_failure_reports_empty_runtime_logs(
    monkeypatch,
    tmp_path: Path,
) -> None:
    """Failed runtime commands should report empty runtime log candidates."""
    module = _module()
    state_dir = tmp_path / ".runtime-smoke"
    worker_log = state_dir / "logs" / "temporal-worker-orchestration.log"
    worker_log.parent.mkdir(parents=True)
    worker_log.touch()
    ...
    try:
        ...
        message = str(err)
        assert "runtime log diagnostics" in message
        assert f"runtime log empty: {worker_log}" in message
    else:  # pragma: no cover - defensive assertion shape
        raise AssertionError("expected failed command to report empty logs")
