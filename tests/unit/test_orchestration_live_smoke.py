"""Tests for operator-gated live orchestration smoke helpers."""
...
import histdatacom.orchestration.live_smoke as live_smoke
...
from histdatacom.orchestration.live_smoke import (
    ...
    default_client_routing_orchestration_smoke_request,
    default_hermetic_orchestration_smoke_request,
    default_live_orchestration_smoke_request,
...
from histdatacom.orchestration.supervisor import OrchestrationStatus
...
class _FakeSupervisor:
    ...
    def stop(self, *, stop_timeout: float = 0.0) -> OrchestrationStatus:
        self.stopped = True
...
def _completed_snapshot(
    ...
    *,
    ...
    return OrchestrationJobSnapshot(
...
def test_default_live_orchestration_smoke_request_is_minimal_non_influx(
    tmp_path: Path,
) -> None:
...
def test_run_hermetic_orchestration_smoke_uses_local_only_request_and_stops(
    tmp_path: Path,
) -> None:
    ...
    def submit_job(
        request: RunRequest, **kwargs: Any
    ) -> OrchestrationJobSnapshot:
        captured["request"] = request
        captured["kwargs"] = kwargs
        return _completed_snapshot(request)

    result = run_hermetic_orchestration_smoke(
    ...
    assert supervisors[0].worker_lanes == DEFAULT_LIVE_ORCHESTRATION_SMOKE_LANES
...
def test_run_default_client_routing_orchestration_smoke_omits_explicit_config(
    tmp_path: Path,
) -> None:
    ...
    result = run_default_client_routing_orchestration_smoke(
        workspace=tmp_path / "workspace",
        runtime_home=tmp_path / "runtime",
        data_directory=tmp_path / "data",
        supervisor_factory=supervisor_factory,
...
def test_run_live_orchestration_smoke_uses_external_request_and_stops(
    tmp_path: Path,
) -> None:
    ...
    result = run_live_orchestration_smoke(
        workspace=tmp_path / "workspace",
        runtime_home=tmp_path / "runtime",
        data_directory=tmp_path / "data",
        supervisor_factory=supervisor_factory,
...
def test_run_live_orchestration_smoke_failure_includes_log_diagnostics(
    tmp_path: Path,
) -> None:
    def supervisor_factory(**kwargs: Any) -> _FakeSupervisor:
    ...
    with pytest.raises(LiveOrchestrationSmokeError) as raised:
        run_live_orchestration_smoke(
            workspace=tmp_path / "workspace",
            runtime_home=tmp_path / "runtime",
            data_directory=tmp_path / "data",
...
def test_run_live_orchestration_smoke_fails_when_stop_raises(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        LiveOrchestrationSmokeError, match="shutdown failed"
    ) as raised:
        run_live_orchestration_smoke(
            workspace=tmp_path / "workspace",
            runtime_home=tmp_path / "runtime",
...
def test_run_live_orchestration_smoke_fails_when_stop_remains_stopping(
    tmp_path: Path,
    ...
    with pytest.raises(
        LiveOrchestrationSmokeError, match="state=stopping"
    ) as raised:
        run_live_orchestration_smoke(
            workspace=tmp_path / "workspace",
            runtime_home=tmp_path / "runtime",
...
def test_run_live_orchestration_smoke_fails_when_stopped_status_has_pids(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        LiveOrchestrationSmokeError, match="remaining pids"
    ) as raised:
        run_live_orchestration_smoke(
            workspace=tmp_path / "workspace",
            runtime_home=tmp_path / "runtime",
...
def test_run_live_orchestration_smoke_fails_when_stop_returns_no_status(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        LiveOrchestrationSmokeError, match="did not return a status"
    ) as raised:
        run_live_orchestration_smoke(
            workspace=tmp_path / "workspace",
            runtime_home=tmp_path / "runtime",
