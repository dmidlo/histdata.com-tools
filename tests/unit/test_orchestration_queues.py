"""Tests for Temporal orchestration task queue configuration."""
...
def test_task_queues_are_workspace_scoped(tmp_path: Path) -> None:
    ...
    left_policy = build_orchestration_runtime_policy(
        workspace=tmp_path / "left" / "project",
        runtime_home=tmp_path / "runtime",
    )
    right_policy = build_orchestration_runtime_policy(
        workspace=tmp_path / "right" / "project",
        runtime_home=tmp_path / "runtime",
    )

    left = build_orchestration_task_queues(runtime_policy=left_policy)
    right = build_orchestration_task_queues(runtime_policy=right_policy)
...
def test_worker_config_accepts_concurrency_overrides(
    ...
    """Orchestration workers should expose lane-level concurrency tuning."""
