...
Automation can rely on the top-level report payload shape, bounded sidecar
...
fields, finding fields, bounded sidecar payload keys, or artifact metadata
...
- bounded sidecar payload with quality-report artifact metadata.
...
breaks the bounded sidecar payload contract, update the schema version and
