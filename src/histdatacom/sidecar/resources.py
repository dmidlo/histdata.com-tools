...
class TemporalRuntimeProvisioningError(SidecarResourceError):
...
def load_temporal_runtime_index(
    ...
    if not loaded_manifest.runtime_artifact_index:
        raise TemporalRuntimeProvisioningError(
            "Sidecar manifest does not declare a Temporal runtime artifact index."
        )
    return TemporalRuntimeIndex.from_dict(
        json.loads(
            read_sidecar_asset_text(loaded_manifest.runtime_artifact_index)
        )
    )
    ...
    *,
    ...
    try:
        with sidecar_executable_path(
            ...
            resource = sidecar_platform_resource(resolved_key, loaded_manifest)
            ...
            return
    except SidecarExecutableUnavailable:
        pass
...
def _validate_packaged_temporal_runtime(
    resource: SidecarPlatformResource,
    executable: Path,
    artifact: TemporalRuntimeArtifact,
    index: TemporalRuntimeIndex,
) -> None:
    ...
    try:
        provenance = json.loads(read_sidecar_asset_text(resource.provenance))
