"""Maintenance and retention helpers for local sidecar runtime state."""
...
class SidecarRetentionPolicy:
    """Bounded retention policy for workspace-scoped sidecar state."""
...
class LogMaintenanceResult:
    """Result for one sidecar log maintenance operation."""
...
@dataclass(frozen=True, slots=True)
class SidecarMaintenanceResult:
    """JSON-ready result for a sidecar maintenance pass."""

    state: str
    message: str
    runtime_policy: SidecarRuntimePolicy
    retention_policy: SidecarRetentionPolicy
    sidecar_state: str
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "data_directory_policy": (
                "Sidecar maintenance is workspace-scoped and does not remove "
...
def run_sidecar_maintenance(
    runtime_policy: SidecarRuntimePolicy,
    retention_policy: SidecarRetentionPolicy | None = None,
    *,
    sidecar_state: str = "stopped",
    allow_running: bool = False,
) -> SidecarMaintenanceResult:
    """Run workspace-scoped sidecar maintenance."""
    policy = retention_policy or SidecarRetentionPolicy()
    temporal_sqlite = _temporal_sqlite_result(runtime_policy, policy)
    warnings = _temporal_sqlite_warnings(temporal_sqlite)
    skip_reason = ""
    if sidecar_state == "running" and not allow_running:
        skip_reason = (
            "Sidecar is running; stop it or pass --allow-running before "
    ...
    if skip_reason:
        return SidecarMaintenanceResult(
            state="skipped",
            message=skip_reason,
    ...
    if errors:
        return SidecarMaintenanceResult(
            state="error",
            message="Sidecar maintenance completed with errors.",
            runtime_policy=runtime_policy,
            retention_policy=policy,
            sidecar_state=sidecar_state,
            logs=logs,
    ...
    return SidecarMaintenanceResult(
        state="completed",
        message="Sidecar maintenance completed.",
        runtime_policy=runtime_policy,
        retention_policy=policy,
        sidecar_state=sidecar_state,
        logs=logs,
...
def _maintain_logs(
    logs_dir: Path,
    policy: SidecarRetentionPolicy,
    *,
    skip_reason: str = "",
) -> tuple[LogMaintenanceResult, ...]:
    paths = _sidecar_log_paths(logs_dir)
    return tuple(
        _log_result(path, policy, skip_reason=skip_reason) for path in paths
    )


def _sidecar_log_paths(logs_dir: Path) -> tuple[Path, ...]:
