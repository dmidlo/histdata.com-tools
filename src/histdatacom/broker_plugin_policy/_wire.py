...
def plain(value: object) -> object:
    ...
    def visit(item: object, depth: int) -> object:
        ...
        if nodes > MAX_NODES or depth > MAX_DEPTH:
            raise ValueError("broker-provider-policy expanded node/depth bound")
        ...
        if type(item) is dict:
            if len(item) > MAX_ITEMS:
                raise ValueError("broker-provider-policy collection bound")
            size += 2 + 2 * len(item)
            output: dict[str, object] = {}
            for key, child in item.items():
                if type(key) is not str or len(key) > 256:
                    raise ValueError(
                        "invalid broker-provider-policy object key"
                    )
                visit(key, depth + 1)
            ...
            if type(item) is str:
                if len(item) > MAX_TEXT:
                    raise ValueError("broker-provider-policy string bound")
            elif type(item) is int:
                if not -(2**63) <= item < 2**63:
                    raise ValueError("broker-provider-policy integer bound")
            elif type(item) is float:
                if not math.isfinite(item):
                    raise ValueError("broker-provider-policy non-finite number")
            elif item is not None and type(item) is not bool:
                raise ValueError("unsupported broker-provider-policy scalar")
...
def load_json(text: str) -> object:
    if type(text) is not str or len(text) > MAX_BYTES:
        raise ValueError("broker-provider-policy JSON byte bound")

    def pairs(items: list[tuple[str, object]]) -> dict[str, object]:
        result: dict[str, object] = {}
        for key, value in items:
            if key in result:
                raise ValueError("duplicate broker-provider-policy JSON key")
            result[key] = value
        return result
    ...
    try:
        ...
        if canonical_json(result) != text:
            raise ValueError("noncanonical broker-provider-policy JSON")
    except (RecursionError, TypeError, OverflowError) as exc:
        raise ValueError("invalid broker-provider-policy JSON") from exc
    return result
...
def identity(kind: str, payload: object) -> str:
    return f"broker-provider-policy-{kind}:sha256:" + sha256(
...
def text(value: str) -> None:
    if not value or value != value.strip() or any(ord(c) < 32 for c in value):
        raise ValueError("invalid broker-provider-policy text")


def digest(value: str) -> None:
    if re.fullmatch(r"[0-9a-f]{64}", value) is None:
        raise ValueError("invalid broker-provider-policy digest")


def identifier(value: str, kind: str | None = None) -> None:
    pattern = r"broker-provider-policy-[a-z-]+:sha256:[0-9a-f]{64}"
    if re.fullmatch(pattern, value) is None or (
        kind is not None
        and not value.startswith(f"broker-provider-policy-{kind}:sha256:")
    ):
        raise ValueError("invalid broker-provider-policy artifact identity")


def ordered(values: tuple[str, ...], *, nonempty: bool = False) -> None:
    if (nonempty and not values) or values != tuple(sorted(set(values))):
        raise ValueError("broker-provider-policy values must be sorted unique")
    for value in values:
        text(value)
...
def _decode(hint: Any, value: object) -> Any:
    ...
    if origin in (Union, UnionType):
        for alternative in args:
            try:
                ...
                pass
        raise ValueError("invalid broker-provider-policy union")
    if origin is tuple:
        if type(value) not in (tuple, list):
            raise ValueError("expected broker-provider-policy tuple")
    ...
    if isinstance(hint, type) and issubclass(hint, Record):
        ...
        if type(value) is not dict:
            raise ValueError("expected broker-provider-policy record")
        return hint.from_payload(cast(dict[str, object], value))
    if isinstance(hint, type) and issubclass(hint, Enum):
        if type(value) is hint:
            return value
        if type(value) is not str:
            raise ValueError("expected broker-provider-policy enum")
        return hint(value)
    if type(value) is not hint:
        raise ValueError("invalid broker-provider-policy scalar type")
    return value
...
class Record:
    ...
    def from_payload(
        ...
        if type(payload) is not dict or set(payload) != {
            ...
            raise ValueError("unknown or missing broker-provider-policy field")
        return cls(**payload)
...
class Artifact(Record):
    ...
    def schema_version(cls) -> str:
        return f"histdatacom.broker-provider-policy-{cls.KIND}.v1"
    ...
    def from_dict(
        ...
        if type(value) is not dict or set(value) != {
            ...
            raise ValueError(
                "unknown or missing broker-provider-policy envelope field"
            )
        ...
        if (
            ...
            raise ValueError("wrong broker-provider-policy schema")
        result = cls.from_payload(cast(dict[str, object], value["payload"]))
        if cast(Artifact, result).artifact_id != value["artifact_id"]:
            raise ValueError("broker-provider-policy content identity mismatch")
        return result
    ...
    def from_json(cls: type[_T], text: str) -> _T:  # noqa: PYI019
        ...
        if type(value) is not dict:
            raise ValueError("expected broker-provider-policy envelope")
