...
def _directory(path: Path) -> None:
    ...
    while True:
        ...
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
            raise ValueError(
                "experiment directory must be regular and nonsymlink"
            )
...
def _read_bytes(path: Path) -> bytes:
    ...
    if not stat.S_ISREG(before.st_mode) or before.st_size > MAX_BYTES:
        raise ValueError("experiment file must be regular and bounded")
    descriptor = os.open(
        path,
        os.O_RDONLY
    ...
    try:
        ...
        if not stat.S_ISREG(opened.st_mode) or (
            ...
            raise ValueError("experiment file changed before read")
        chunks: list[bytes] = []
        count = 0
        while True:
            ...
            count += len(chunk)
            if count > MAX_BYTES:
                raise ValueError("experiment file byte bound")
        ...
        if (
            ...
            raise ValueError("experiment file changed during read")
        return b"".join(chunks)
    finally:
        os.close(descriptor)


def read_experiment_artifact(path: str | Path, artifact_type: type[_A]) -> _A:
    ...
    try:
        ...
        if source.name != artifact_filename(result):
            raise ValueError("experiment content filename mismatch")
        return result
    except (OSError, UnicodeError) as exc:
        raise ValueError(
            "unable to read canonical experiment artifact"
        ) from exc
...
def write_experiment_artifact(artifact: _A, directory: str | Path) -> Path:
    ...
    descriptor, temporary = tempfile.mkstemp(prefix=".experiment-", dir=target)
    temporary_path = Path(temporary)
    try:
        with os.fdopen(descriptor, "wb") as handle:
            if handle.write(encoded) != len(encoded):
                raise ValueError("short experiment artifact write")
            handle.flush()
