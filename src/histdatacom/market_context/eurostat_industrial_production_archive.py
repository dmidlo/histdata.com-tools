"""Deterministic qualification of Eurostat monthly industrial production releases.
...
``sts_inpr_m`` JSON-stat table is retained only as a revised-series cross-check;
...
EUROSTAT_INDUSTRIAL_PRODUCTION_SOURCE_KEY: Final = (
    "ea.eurostat.industrial-production"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_PROGRAM_KEY: Final = (
    "ea.eurostat.monthly-industrial-production"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_MEASURE_KEY: Final = (
    "industrial-production-total-industry"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_PARSER_ID: Final = (
    "official.eurostat-industrial-production.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_PARSER_VERSION: Final = "1"
EUROSTAT_INDUSTRIAL_PRODUCTION_SOURCE_TIMEZONE: Final = "Europe/Brussels"
EUROSTAT_INDUSTRIAL_PRODUCTION_AS_OF_DATE: Final = "2026-09-18"
EUROSTAT_INDUSTRIAL_PRODUCTION_FIRST_SEARCH_RELEASE_DATE: Final = "2002-01-21"
EUROSTAT_INDUSTRIAL_PRODUCTION_LATEST_PACKAGED_RELEASE_DATE: Final = (
    "2026-09-16"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_FIRST_REFERENCE_PERIOD: Final = "2001-11"
EUROSTAT_INDUSTRIAL_PRODUCTION_LATEST_REFERENCE_PERIOD: Final = "2026-07"
EUROSTAT_INDUSTRIAL_PRODUCTION_INTERNAL_GAP_REFERENCE_PERIODS: Final = ()
EUROSTAT_INDUSTRIAL_PRODUCTION_PAGE_DATE_OFFSET_EXCEPTIONS: Final = {
    "4-1703204-bp": 4
}
EUROSTAT_INDUSTRIAL_PRODUCTION_REFERENCE_PERIOD_INFERENCES: Final = {
    "4-1703204-bp": "2004-01"
}
EUROSTAT_INDUSTRIAL_PRODUCTION_PERIOD_HEADER_CORRECTIONS: Final = {
    "4-14092020-ap": (
        (
            "2020-02",
            "2020-03",
...
EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_PAGE_COUNT: Final = 13
EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_RESULT_COUNT: Final = 1_213
EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_UNIQUE_URI_COUNT: Final = 1_212
EUROSTAT_INDUSTRIAL_PRODUCTION_TITLE_MATCH_COUNT: Final = 297
EUROSTAT_INDUSTRIAL_PRODUCTION_PREFILTER_COUNT: Final = 297
EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASE_COUNT: Final = 297
EUROSTAT_INDUSTRIAL_PRODUCTION_DOCUMENT_COUNT: Final = 262

EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_URI: Final = (
    "https://ec.europa.eu/eurostat/search"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_PRODUCT_URI_TEMPLATE: Final = (
    "https://ec.europa.eu/eurostat/product?code={product_code}"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_DATASET_URI: Final = (
    "https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/"
    "sts_inpr_m?lang=en&indic_bt=PRD&nace_r2=B-D&"
    "s_adj=SCA&s_adj=CA&unit=PCH_PRE&unit=PCH_SM&"
    "geo=EA21&geo=DE&geo=FR&sinceTimePeriod=2000-01"
)

EUROSTAT_INDUSTRIAL_PRODUCTION_ARTIFACT_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-artifact.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_INVENTORY_ENTRY_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-inventory-entry.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_EXCLUSION_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-exclusion.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_INVENTORY_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-inventory.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_OBSERVATION_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-observation.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_DATASET_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-dataset.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASE_VALUE_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-release-value.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_PUBLISHED_VALUE_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-published-value.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASE_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-release.v1"
)
EUROSTAT_INDUSTRIAL_PRODUCTION_MANIFEST_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-industrial-production-archive-manifest.v1"
)

MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_INDEX_PAGES: Final = 32
MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_ENTRIES: Final = 2_048
MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASES: Final = 512
MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_OBSERVATIONS: Final = 4_096
MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_ARTIFACTS: Final = 2_048
MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_TOTAL_BYTES: Final = 1_024_000_000
MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_TITLE_CHARS: Final = 512
MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_PUBLISHED_VALUES_PER_RELEASE: Final = 96
...
def _product_code(uri_or_code: object) -> str:
    ...
    if parsed.scheme and parsed.path == "/eurostat/product":
        ...
        if len(candidates) != 1:
            raise ValueError(
                "Eurostat industrial production product URI has no unique product code"
            )
        token = candidates[0]
    else:
    ...
    if match is None:
        raise ValueError(
            "Eurostat industrial production product code is invalid"
        )
...
def _inventory_prefilter(title: str) -> bool:
    ...
    return normalized.startswith("industrial production ")
...
def _exclusion_reason(title: str) -> str:
    ...
    if normalized.startswith("services production"):
        return "services-production release, not industrial production"
    if "gdp" in normalized or "employment" in normalized:
        return "national-accounts or labour release, not industrial production"
    if "trade" in normalized:
        return "external-trade release, not industrial production"
    return "broad search-summary match outside the monthly headline lineage"


class EurostatIndustrialProductionArtifactRole(str, Enum):
    ...
    @classmethod
    def from_value(
        cls, value: str | EurostatIndustrialProductionArtifactRole
    ) -> EurostatIndustrialProductionArtifactRole:
        try:
            return value if isinstance(value, cls) else cls(value)
        except ValueError as exc:
            raise ValueError(
                "unsupported Eurostat industrial production artifact role"
            ) from exc


class EurostatIndustrialProductionMeasure(str, Enum):
    """Qualified industrial production statistic carried by this archive."""
    ...
    @classmethod
    def from_value(
        cls, value: str | EurostatIndustrialProductionMeasure
    ) -> EurostatIndustrialProductionMeasure:
        try:
            return value if isinstance(value, cls) else cls(value)
        except ValueError as exc:
            raise ValueError(
                "unsupported Eurostat industrial production measure"
            ) from exc


@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionArtifactV1:
    """Content-addressed official industrial production evidence artifact."""

    role: EurostatIndustrialProductionArtifactRole
    request_uri: str
    resolved_uri: str
    ...
    schema_version: str = EUROSTAT_INDUSTRIAL_PRODUCTION_ARTIFACT_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production artifact schema"
            )
        role = EurostatIndustrialProductionArtifactRole.from_value(self.role)
        ...
        expected_format = {
            EurostatIndustrialProductionArtifactRole.SEARCH_INDEX: OfficialSourceFormat.ATOM,
            EurostatIndustrialProductionArtifactRole.CURRENT_DATASET: OfficialSourceFormat.JSON_STAT,
            EurostatIndustrialProductionArtifactRole.RELEASE_HTML: OfficialSourceFormat.HTML,
            EurostatIndustrialProductionArtifactRole.RELEASE_DOCUMENT_HTML: OfficialSourceFormat.HTML,
            EurostatIndustrialProductionArtifactRole.RELEASE_DOCUMENT_PDF: OfficialSourceFormat.PDF,
        }[role]
        if source_format is not expected_format:
            raise ValueError(
                "Eurostat industrial production artifact role and format differ"
            )
        length = _bounded_int(
            self.content_length,
            "content_length",
            MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_TOTAL_BYTES,
        )
        if length < 1:
            raise ValueError("Eurostat industrial production artifact is empty")
        page_number = self.page_number
        product_code = self.product_code
        if role is EurostatIndustrialProductionArtifactRole.SEARCH_INDEX:
            if page_number is None:
                raise ValueError(
                    "Eurostat industrial production search artifact needs a page number"
                )
            _bounded_int(
                page_number,
                "page_number",
                MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_INDEX_PAGES,
            )
            if product_code is not None:
                raise ValueError(
                    "Eurostat industrial production search artifact has a product code"
                )
        elif role is EurostatIndustrialProductionArtifactRole.CURRENT_DATASET:
            if page_number is not None or product_code is not None:
                raise ValueError(
                    "Eurostat industrial production dataset artifact has release metadata"
                )
        else:
            if page_number is not None or product_code is None:
                raise ValueError(
                    "Eurostat industrial production release artifact metadata differs"
                )
    ...
    def from_dict(
        ...
        return cls(
            role=EurostatIndustrialProductionArtifactRole.from_value(
                str(data.get("role", ""))
            ),
...
@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionInventoryEntryV1:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production inventory-entry schema"
            )
        ...
        if release_date != _product_release_date(product_code):
            raise ValueError(
                "Eurostat industrial production release date differs from product code"
            )
        title = _required_text(
            self.source_title,
            "source_title",
            maximum=MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_TITLE_CHARS,
        )
        if not _inventory_selected(title, product_code):
            raise ValueError(
                "Eurostat industrial production inventory entry is outside selection"
            )
        ...
        if (
            ...
            raise ValueError(
                "Eurostat industrial production source URI route differs"
            )
        if _product_code(source_uri) != product_code:
            raise ValueError(
                "Eurostat industrial production source URI and product code differ"
            )
        page = _bounded_int(
            self.page_number,
            "page_number",
        ...
        if page < 1:
            raise ValueError(
                "Eurostat industrial production page number must be positive"
            )
        object.__setattr__(self, "product_code", product_code)
        ...
        expected = _stable_id(
            "eurostat-industrial-production-inventory-entry",
            self.identity_payload(),
        )
        if self.entry_id and self.entry_id != expected:
            raise ValueError(
                "Eurostat industrial production inventory-entry identity differs"
            )
...
@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionIndexExclusionV1:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production exclusion schema"
            )
        product_code = _product_code(self.product_code)
        release_date = _iso_date(self.release_date, "release_date")
        if release_date != _product_release_date(product_code):
            raise ValueError(
                "Eurostat industrial production exclusion date differs"
            )
        title = _required_text(
            self.source_title,
            "source_title",
        ...
        if _inventory_selected(title, product_code):
            raise ValueError(
                "Eurostat industrial production exclusion matches selection"
            )
        source_uri = _https_uri(self.source_uri, "source_uri")
        if _product_code(source_uri) != product_code:
            raise ValueError(
                "Eurostat industrial production exclusion URI differs"
            )
        reason = _required_text(self.reason, "reason")
        if reason != _exclusion_reason(title):
            raise ValueError(
                "Eurostat industrial production exclusion reason differs"
            )
        object.__setattr__(self, "product_code", product_code)
        object.__setattr__(self, "release_date", release_date)
        object.__setattr__(self, "source_title", title)
        ...
        expected = _stable_id(
            "eurostat-industrial-production-exclusion", self.identity_payload()
        )
        if self.exclusion_id and self.exclusion_id != expected:
            raise ValueError(
                "Eurostat industrial production exclusion identity differs"
            )
...
@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionInventoryV1:
    """Hash-bound official search inventory and deterministic selection."""

    artifacts: tuple[EurostatIndustrialProductionArtifactV1, ...]
    entries: tuple[EurostatIndustrialProductionInventoryEntryV1, ...]
    exclusions: tuple[EurostatIndustrialProductionIndexExclusionV1, ...]
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production inventory schema"
            )
        artifacts = tuple(self.artifacts)
        entries = tuple(self.entries)
        exclusions = tuple(self.exclusions)
        if len(artifacts) != EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_PAGE_COUNT:
            raise ValueError(
                "Eurostat industrial production search page count differs"
            )
        if any(
            item.role
            is not EurostatIndustrialProductionArtifactRole.SEARCH_INDEX
            for item in artifacts
        ):
            raise ValueError(
                "Eurostat industrial production inventory artifact role differs"
            )
        if [item.page_number for item in artifacts] != list(
            range(1, EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_PAGE_COUNT + 1)
        ):
            raise ValueError(
                "Eurostat industrial production search pages are not contiguous"
            )
        if any(
            ...
            raise ValueError(
                "Eurostat industrial production search artifact identity differs"
            )
        if len(entries) != EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASE_COUNT:
            raise ValueError(
                "Eurostat industrial production selected release count differs"
            )
        codes = [item.product_code for item in entries]
        if len(codes) != len(set(codes)):
            raise ValueError(
                "Eurostat industrial production inventory repeats a product code"
            )
        if list(entries) != sorted(
            entries, key=lambda item: (item.release_date, item.product_code)
        ):
            raise ValueError(
                "Eurostat industrial production inventory entries are not canonical"
            )
        if (
            ...
            raise ValueError(
                "Eurostat industrial production first searchable release differs"
            )
        if (
            ...
            raise ValueError(
                "Eurostat industrial production latest searchable release differs"
            )
        if len(exclusions) != (
            ...
            raise ValueError(
                "Eurostat industrial production exclusion count differs"
            )
        exclusion_codes = [item.product_code for item in exclusions]
        if len(exclusion_codes) != len(set(exclusion_codes)):
            raise ValueError(
                "Eurostat industrial production exclusions repeat a product"
            )
        if list(exclusions) != sorted(
            exclusions, key=lambda item: (item.release_date, item.product_code)
        ):
            raise ValueError(
                "Eurostat industrial production exclusions are not canonical"
            )
        if set(codes) & set(exclusion_codes):
            raise ValueError(
                "Eurostat industrial production selection includes an exclusion"
            )
        expected_counts = (
            EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_RESULT_COUNT,
            EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_UNIQUE_URI_COUNT,
            EUROSTAT_INDUSTRIAL_PRODUCTION_TITLE_MATCH_COUNT,
            EUROSTAT_INDUSTRIAL_PRODUCTION_PREFILTER_COUNT,
        )
        observed_counts = (
            _bounded_int(
                self.query_result_count,
                "query_result_count",
                MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_ENTRIES,
            ),
            _bounded_int(
        ...
        if observed_counts != expected_counts:
            raise ValueError(
                "Eurostat industrial production search inventory counts differ"
            )
        object.__setattr__(self, "artifacts", artifacts)
        object.__setattr__(self, "entries", entries)
        object.__setattr__(self, "exclusions", exclusions)
        expected = _stable_id(
            "eurostat-industrial-production-inventory", self.identity_payload()
        )
        if self.inventory_id and self.inventory_id != expected:
            raise ValueError(
                "Eurostat industrial production inventory identity differs"
            )
    ...
    def from_dict(
        ...
        return cls(
            artifacts=tuple(
                EurostatIndustrialProductionArtifactV1.from_dict(
                    _mapping(item, "artifact")
                )
                for item in _sequence(data.get("artifacts"), "artifacts")
            ),
            entries=tuple(
                EurostatIndustrialProductionInventoryEntryV1.from_dict(
                    _mapping(item, "inventory entry")
                )
                for item in _sequence(data.get("entries"), "entries")
            ),
            exclusions=tuple(
                EurostatIndustrialProductionIndexExclusionV1.from_dict(
...
@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionObservationV1:
    """One current-revised JSON-stat industrial-production observation."""

    economy_code: str
    reference_period: str
    measure: EurostatIndustrialProductionMeasure
    value_lexical: str
    value: float
    status: str | None
    flat_index: int
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production observation schema"
            )
        economy = _required_text(self.economy_code, "economy_code")
        if economy not in _ECONOMIES:
            raise ValueError(
                "Eurostat industrial production observation economy differs"
            )
        reference = _month(self.reference_period, "reference_period")
        measure = EurostatIndustrialProductionMeasure.from_value(self.measure)
        lexical = _required_text(self.value_lexical, "value_lexical")
        if _NUMBER_RE.fullmatch(lexical) is None:
            raise ValueError(
                "Eurostat industrial production observation lexical is invalid"
            )
        numeric = float(lexical)
        if numeric != self.value or not math.isfinite(numeric):
            raise ValueError(
                "Eurostat industrial production observation numeric differs"
            )
        if not -100.0 <= numeric <= 100.0:
            raise ValueError(
                "Eurostat industrial production observation is outside its bound"
            )
        status = _optional_text(self.status, "status")
        if status is not None and len(status) > 16:
            raise ValueError(
                "Eurostat industrial production status exceeds its bound"
            )
        flat_index = _bounded_int(
            self.flat_index,
            "flat_index",
            MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_OBSERVATIONS * 4,
        )
        ...
        expected = _stable_id(
            "eurostat-industrial-production-observation",
            self.identity_payload(),
        )
        if self.observation_id and self.observation_id != expected:
            raise ValueError(
                "Eurostat industrial production observation identity differs"
            )
    ...
    def from_dict(
        ...
        return cls(
            ...
            measure=EurostatIndustrialProductionMeasure.from_value(
...
@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionDatasetV1:
    """Exact current-revised ``sts_inpr_m`` cross-check response."""

    label: str
    updated_at: str
    time_start: str
    time_end: str
    artifact: EurostatIndustrialProductionArtifactV1
    observations: tuple[EurostatIndustrialProductionObservationV1, ...]
    dataset_id: str = "sts_inpr_m"
    dataset_receipt_id: str = ""
    schema_version: str = EUROSTAT_INDUSTRIAL_PRODUCTION_DATASET_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production dataset schema"
            )
        if self.dataset_id != "sts_inpr_m":
            raise ValueError(
                "unsupported Eurostat industrial production dataset"
            )
        ...
        if time_end < time_start:
            raise ValueError(
                "Eurostat industrial production dataset time range is reversed"
            )
        if (
            self.artifact.role
            is not EurostatIndustrialProductionArtifactRole.CURRENT_DATASET
        ):
            raise ValueError(
                "Eurostat industrial production dataset artifact role differs"
            )
        if (
            ...
            raise ValueError(
                "Eurostat industrial production dataset artifact identity differs"
            )
        observations = tuple(self.observations)
        if (
            not 1
            <= len(observations)
            <= MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_OBSERVATIONS
        ):
            raise ValueError(
                "Eurostat industrial production observation count is invalid"
            )
        keys = [item.key for item in observations]
        if len(keys) != len(set(keys)):
            raise ValueError(
                "Eurostat industrial production dataset repeats an observation"
            )
        ...
        if missing_keys or set(keys) - expected_keys:
            raise ValueError(
                "Eurostat industrial production dataset cube coverage differs"
            )
        period_positions = {
            period: position for position, period in enumerate(periods)
        }
        measure_coordinates = {
            EurostatIndustrialProductionMeasure.MONTH_OVER_MONTH: (1, 0),
            EurostatIndustrialProductionMeasure.YEAR_OVER_YEAR: (0, 1),
        }
        if any(
            item.flat_index
            != (
            ...
            raise ValueError(
                "Eurostat industrial production observation coordinate differs"
            )
        if list(observations) != sorted(
            observations,
            key=lambda item: (
                list(EurostatIndustrialProductionMeasure).index(item.measure),
                _ECONOMY_ORDER[item.economy_code],
                item.reference_period,
            ),
        ):
            raise ValueError(
                "Eurostat industrial production observations are not canonical"
            )
        if (
            ...
            raise ValueError(
                "Eurostat industrial production dataset time range differs"
            )
        ...
        expected = _stable_id(
            "eurostat-industrial-production-dataset", self.identity_payload()
        )
        if self.dataset_receipt_id and self.dataset_receipt_id != expected:
            raise ValueError(
                "Eurostat industrial production dataset identity differs"
            )
    ...
    def from_dict(
        ...
        return cls(
            ...
            artifact=EurostatIndustrialProductionArtifactV1.from_dict(
                _mapping(data.get("artifact"), "artifact")
            ),
            observations=tuple(
                EurostatIndustrialProductionObservationV1.from_dict(
...
def _search_uri(page_number: int) -> str:
    ...
    if page < 1:
        raise ValueError(
            "Eurostat industrial production search page must be positive"
        )
    params = {
        "p_p_id": (
        ...
        f"{_SEARCH_PREFIX}text": "industrial production",
        f"{_SEARCH_PREFIX}sort": "date",
        f"{_SEARCH_PREFIX}collection": "CAT_PREREL",
        f"{_SEARCH_PREFIX}priv_r_p_implicitModel": "true",
        "pageNumber": str(page),
    }
    return f"{EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_URI}?{urlencode(params)}"
...
def _request(
    ...
    *,
    ...
    source = registry.source(EUROSTAT_INDUSTRIAL_PRODUCTION_SOURCE_KEY)
...
def build_eurostat_industrial_production_search_requests(
    registry: OfficialSourceRegistryV1,
    *,
    page_count: int = EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_PAGE_COUNT,
) -> tuple[OfficialSourceRequestV1, ...]:
    """Build the bounded official Atom inventory requests."""
    count = _bounded_int(
        page_count, "page_count", MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_INDEX_PAGES
    )
    if count < 1:
        raise ValueError(
            "Eurostat industrial production search needs at least one page"
        )
    return tuple(
        _request(
            registry,
...
def build_eurostat_industrial_production_dataset_request(
    registry: OfficialSourceRegistryV1,
) -> OfficialSourceRequestV1:
    """Build the exact current-revised industrial production cross-check request."""
    return _request(
        registry,
        EUROSTAT_INDUSTRIAL_PRODUCTION_DATASET_URI,
...
def build_eurostat_industrial_production_release_requests(
    registry: OfficialSourceRegistryV1,
    inventory: EurostatIndustrialProductionInventoryV1,
) -> tuple[OfficialSourceRequestV1, ...]:
    """Build one primary request for every selected industrial production product."""
    if not isinstance(inventory, EurostatIndustrialProductionInventoryV1):
        raise TypeError(
            "Eurostat industrial production release requests require a v1 inventory"
        )
    return tuple(
...
def _document_source_format(uri: str) -> OfficialSourceFormat:
    ...
    raise ValueError(
        "Eurostat industrial production release document format differs"
    )


def build_eurostat_industrial_production_document_requests(
    registry: OfficialSourceRegistryV1,
    inventory: EurostatIndustrialProductionInventoryV1,
    ...
    if not isinstance(inventory, EurostatIndustrialProductionInventoryV1):
        raise TypeError(
            "Eurostat industrial production document requests require a v1 inventory"
        )
    entries = {item.product_code: item for item in inventory.entries}
    snapshots = tuple(release_snapshots)
    if len(snapshots) != EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASE_COUNT:
        raise ValueError(
            "Eurostat industrial production landing snapshot count differs"
        )
    ...
    for snapshot in snapshots:
        ...
        if code in by_code:
            raise ValueError(
                "Eurostat industrial production landing corpus repeats a product code"
            )
        by_code[code] = snapshot
    if set(by_code) != set(entries):
        raise ValueError(
            "Eurostat industrial production landing corpus is incomplete"
        )
    ...
    for code, entry in entries.items():
        ...
        if snapshot.request.uri != entry.source_uri:
            raise ValueError(
                "Eurostat industrial production landing request identity differs"
            )
    ...
    if (
        ...
        and len(requests) != EUROSTAT_INDUSTRIAL_PRODUCTION_DOCUMENT_COUNT
    ):
        raise ValueError(
            "Eurostat industrial production English document count differs"
        )
...
def _artifact_from_snapshot(
    ...
    *,
    ...
    if not isinstance(snapshot, OfficialRawSnapshotV1):
        raise TypeError(
            "Eurostat industrial production artifact requires a v1 snapshot"
        )
    if (
        snapshot.request.source_key != EUROSTAT_INDUSTRIAL_PRODUCTION_SOURCE_KEY
        ...
        raise ValueError(
            "Eurostat industrial production snapshot source or parser differs"
        )
    if snapshot.request.method is not OfficialRequestMethod.GET:
        raise ValueError(
            "Eurostat industrial production snapshot method differs"
        )
    expected_format = {
        EurostatIndustrialProductionArtifactRole.SEARCH_INDEX: OfficialSourceFormat.ATOM,
        EurostatIndustrialProductionArtifactRole.CURRENT_DATASET: OfficialSourceFormat.JSON_STAT,
    ...
    if snapshot.request.source_format is not expected_format:
        raise ValueError(
            "Eurostat industrial production snapshot format differs from artifact role"
        )
    if snapshot.status_code != 200:
        raise ValueError(
            "Eurostat industrial production snapshot status differs"
        )
    if role is EurostatIndustrialProductionArtifactRole.SEARCH_INDEX:
        if (
            ...
            raise ValueError(
                "Eurostat industrial production search request identity differs"
            )
        try:
            root = safe_xml_fromstring(snapshot.content)
        except (DefusedXmlException, ParseError) as exc:
            raise ValueError(
                "Eurostat industrial production search artifact is invalid XML"
            ) from exc
        if root.tag != f"{{{_ATOM_NAMESPACE}}}feed":
            raise ValueError(
                "Eurostat industrial production search artifact is not Atom"
            )
    elif role is EurostatIndustrialProductionArtifactRole.CURRENT_DATASET:
        if (
            ...
            raise ValueError(
                "Eurostat industrial production dataset request identity differs"
            )
        if not snapshot.content.lstrip().startswith(b"{"):
            raise ValueError(
                "Eurostat industrial production dataset artifact is not JSON"
            )
    elif snapshot.request.page_number is not None:
        raise ValueError(
            "Eurostat industrial production release request has a page number"
        )
    elif role in {
        EurostatIndustrialProductionArtifactRole.RELEASE_HTML,
        EurostatIndustrialProductionArtifactRole.RELEASE_DOCUMENT_HTML,
    }:
        if b"<html" not in snapshot.content[:16_384].lower():
            raise ValueError(
                "Eurostat industrial production HTML signature differs"
            )
    elif not snapshot.content.startswith(b"%PDF-"):
        raise ValueError("Eurostat industrial production PDF signature differs")
    if role in {
        EurostatIndustrialProductionArtifactRole.RELEASE_DOCUMENT_HTML,
        ...
        raise ValueError(
            "Eurostat industrial production document product code differs"
        )
    return EurostatIndustrialProductionArtifactV1(
        role=role,
...
def _atom_text(entry: Element, tag: str) -> str:
    ...
    if node is None or node.text is None:
        raise ValueError(
            f"Eurostat industrial production Atom entry omits {tag}"
        )
...
def _atom_link(entry: Element) -> str:
    ...
    if len(links) != 1 or "href" not in links[0].attrib:
        raise ValueError(
            "Eurostat industrial production Atom entry alternate link differs"
        )
    return _https_uri(links[0].attrib["href"], "source_uri")


def parse_eurostat_industrial_production_inventory(
    snapshots: Sequence[OfficialRawSnapshotV1],
    *,
    as_of_date: str,
) -> EurostatIndustrialProductionInventoryV1:
    """Parse and prove the complete bounded industrial production search selection."""
    as_of = _iso_date(as_of_date, "as_of_date")
    values = tuple(snapshots)
    if len(values) != EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_PAGE_COUNT:
        raise ValueError(
            "Eurostat industrial production inventory snapshot count differs"
        )
    if [item.request.page_number for item in values] != list(
        range(1, EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_PAGE_COUNT + 1)
    ):
        raise ValueError(
            "Eurostat industrial production search snapshots are not contiguous"
        )
    artifacts: list[EurostatIndustrialProductionArtifactV1] = []
    raw_entries: list[tuple[int, str, str, str, str, str]] = []
    for snapshot in values:
        artifact = _artifact_from_snapshot(
            snapshot, EurostatIndustrialProductionArtifactRole.SEARCH_INDEX
        )
        ...
        try:
            ...
            raise ValueError(
                "Eurostat industrial production search page is invalid XML"
            ) from exc
        ...
        if len(entries) != expected_count:
            raise ValueError(
                "Eurostat industrial production search page size differs"
            )
    ...
    if len(raw_entries) != EUROSTAT_INDUSTRIAL_PRODUCTION_SEARCH_RESULT_COUNT:
        raise ValueError(
            "Eurostat industrial production search result count differs"
        )
    ...
    for item in raw_entries:
        ...
        if previous is not None and previous[1:] != item[1:]:
            raise ValueError(
                "Eurostat industrial production search duplicate URI differs"
            )
    ...
    if (
        ...
        raise ValueError(
            "Eurostat industrial production search unique URI count differs"
        )
    unique_entries = tuple(unique_by_uri.values())
    title_match_count = sum(
        re.search(r"\bindustrial production\b", title, re.IGNORECASE)
        is not None
    ...
    selected: list[EurostatIndustrialProductionInventoryEntryV1] = []
    exclusions: list[EurostatIndustrialProductionIndexExclusionV1] = []
    ...
    return EurostatIndustrialProductionInventoryV1(
...
def parse_eurostat_industrial_production_dataset(
    snapshot: OfficialRawSnapshotV1,
) -> EurostatIndustrialProductionDatasetV1:
    """Decode the exact current-revised industrial production JSON-stat cross-check."""
    artifact = _artifact_from_snapshot(
        snapshot, EurostatIndustrialProductionArtifactRole.CURRENT_DATASET
    )
    try:
        ...
        raise ValueError(
            "Eurostat industrial production dataset is invalid JSON"
        ) from exc
    ...
    if identifiers != expected_ids:
        raise ValueError(
            "Eurostat industrial production dataset dimensions differ"
        )
    ...
    if any(
        ...
        raise TypeError(
            "Eurostat industrial production dimension sizes must be integers"
        )
    ...
    if observed_codes != expected_codes:
        raise ValueError(
            "Eurostat industrial production dataset dimensions or codes differ"
        )
    ...
    if not time_codes or any(
        ...
        raise ValueError(
            "Eurostat industrial production dataset time codes differ"
        )
    if time_codes != _month_range(time_codes[0], time_codes[-1]):
        raise ValueError(
            "Eurostat industrial production dataset time codes are not contiguous"
        )
    expected_sizes = (1, 1, 1, 2, 2, len(_ECONOMIES), len(time_codes))
    if tuple(cast(Sequence[int], sizes)) != expected_sizes:
        raise ValueError("Eurostat industrial production dataset sizes differ")
    ...
    if set(values) != set(lexical_values):
        raise ValueError(
            "Eurostat industrial production lexical and numeric values differ"
        )
    if not set(statuses) <= set(values):
        raise ValueError(
            "Eurostat industrial production status has no observation"
        )
    observations: list[EurostatIndustrialProductionObservationV1] = []
    ...
    for raw_index, raw_value in values.items():
        try:
            ...
            raise ValueError(
                "Eurostat industrial production sparse index is invalid"
            ) from exc
        if not 0 <= flat_index < maximum:
            raise ValueError(
                "Eurostat industrial production sparse index is outside its cube"
            )
        ...
        measure = {
            (1, 0): EurostatIndustrialProductionMeasure.MONTH_OVER_MONTH,
            (0, 1): EurostatIndustrialProductionMeasure.YEAR_OVER_YEAR,
        }.get((s_adj_index, unit_index))
        if measure is None:
            raise ValueError(
                "Eurostat industrial production dataset populated an unsupported "
    ...
    return EurostatIndustrialProductionDatasetV1(
...
class EurostatIndustrialProductionMovement(str, Enum):
    ...
    @classmethod
    def from_value(
        cls, value: str | EurostatIndustrialProductionMovement
    ) -> EurostatIndustrialProductionMovement:
        try:
            return value if isinstance(value, cls) else cls(value)
        except ValueError as exc:
            raise ValueError(
                "unsupported Eurostat industrial production movement"
            ) from exc


@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionReleaseValueV1:
    """Source-era euro-area industrial production rate in a release headline."""

    economy_code: str
    reference_period: str
    measure: EurostatIndustrialProductionMeasure
    movement: EurostatIndustrialProductionMovement
    value_lexical: str
    value: float
    evidence_id: str
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production release-value schema"
            )
        if self.economy_code != "EA":
            raise ValueError(
                "Eurostat industrial production release value must retain source-era EA scope"
            )
        reference = _month(self.reference_period, "reference_period")
        measure = EurostatIndustrialProductionMeasure.from_value(self.measure)
        if measure is not EurostatIndustrialProductionMeasure.MONTH_OVER_MONTH:
            raise ValueError(
                "Eurostat industrial production headline measure differs"
            )
        movement = EurostatIndustrialProductionMovement.from_value(
            self.movement
        )
        lexical = _required_text(self.value_lexical, "value_lexical")
        if _NUMBER_RE.fullmatch(lexical) is None or lexical.startswith("-"):
            raise ValueError(
                "Eurostat industrial production headline rate is invalid"
            )
        magnitude = float(lexical)
        numeric = {
            EurostatIndustrialProductionMovement.DOWN: -magnitude,
            EurostatIndustrialProductionMovement.STABLE: 0.0,
        }.get(movement, magnitude)
        if self.value != numeric or not math.isfinite(self.value):
            raise ValueError(
                "Eurostat industrial production headline numeric value differs"
            )
        if not -100.0 <= self.value <= 100.0:
            raise ValueError(
                "Eurostat industrial production headline value is outside its bound"
            )
        evidence_id = _required_text(self.evidence_id, "evidence_id")
        if not evidence_id.startswith(
            "eurostat-industrial-production-inventory-entry:sha256:"
        ):
            raise ValueError(
                "Eurostat industrial production headline evidence differs"
            )
        object.__setattr__(self, "reference_period", reference)
        object.__setattr__(self, "measure", measure)
        ...
        expected = _stable_id(
            "eurostat-industrial-production-release-value",
            self.identity_payload(),
        )
        if self.release_value_id and self.release_value_id != expected:
            raise ValueError(
                "Eurostat industrial production release-value identity differs"
            )
...
@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionPublishedValueV1:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production published-value schema"
            )
        economy = _required_text(self.economy_code, "economy_code")
        if economy not in _RELEASE_ECONOMIES:
            raise ValueError(
                "Eurostat industrial production published-value economy differs"
            )
        reference = _month(self.reference_period, "reference_period")
        measure = EurostatIndustrialProductionMeasure.from_value(self.measure)
        lexical = _required_text(self.value_lexical, "value_lexical")
        if _SIGNED_NUMBER_RE.fullmatch(lexical) is None:
            raise ValueError(
                "Eurostat industrial production published-value lexical is invalid"
            )
        numeric = float(lexical)
        if numeric != self.value or not math.isfinite(numeric):
            raise ValueError(
                "Eurostat industrial production published-value numeric differs"
            )
        if not -100.0 <= numeric <= 100.0:
            raise ValueError(
                "Eurostat industrial production published value is outside its bound"
            )
        digest = _sha256(
        ...
        expected = _stable_id(
            "eurostat-industrial-production-published-value",
            self.identity_payload(),
        )
        if self.published_value_id and self.published_value_id != expected:
            raise ValueError(
                "Eurostat industrial production published-value identity differs"
            )
...
def _release_lag(release_date: str, reference_period: str) -> int:
    ...
    if not 20 <= lag <= 70:
        raise ValueError(
            "Eurostat industrial production release lag is outside the qualified window"
        )
    return lag


_HEADLINE_VALUE_RE = re.compile(
    r"\bindustrial production\s+"
    r"(?:(?P<movement>up|down)\s+by\s+"
    r"(?P<value>\d+(?:\.\d+)?)\s*%|(?P<stable>stable|unchanged)\b)",
    re.IGNORECASE,
)


def _headline_value(
    entry: EurostatIndustrialProductionInventoryEntryV1,
    reference_period: str,
) -> EurostatIndustrialProductionReleaseValueV1:
    match = _HEADLINE_VALUE_RE.search(entry.source_title)
    if match is None:
        raise ValueError(
            "Eurostat industrial production release headline has no rate"
        )
    movement_token = (
    ...
    movement = (
        ...
        else EurostatIndustrialProductionMovement.from_value(movement_token)
    )
    ...
    return EurostatIndustrialProductionReleaseValueV1(
        economy_code="EA",
...
def _parse_release_landing(
    ...
    try:
        ...
        raise ValueError(
            "Eurostat industrial production release landing is not UTF-8"
        ) from exc
    ...
    if titles != (entry.source_title,):
        raise ValueError(
            "Eurostat industrial production Atom and landing titles differ"
        )
    ...
    for date_match in _LANDING_DATE_RE.finditer(text):
        ...
        if month is None:
            raise ValueError(
                "Eurostat industrial production landing month is unsupported"
            )
        try:
            page_dates.add(
                date(
            ...
            raise ValueError(
                "Eurostat industrial production landing date is invalid"
            ) from exc
    ...
    if len(published_values) > 1:
        raise ValueError(
            "Eurostat industrial production release landing has no unique publication time"
        )
    ...
    if len(page_dates) != 1:
        raise ValueError(
            "Eurostat industrial production release landing has no unique release date"
        )
    ...
    if len(document_uris) > 1:
        raise ValueError(
            "Eurostat industrial production landing exposes multiple English release documents"
        )
    ...
    if snapshot.request.source_format is OfficialSourceFormat.HTML:
        ...
        try:
            ...
            if len(failed_pages) > 1 or not any(
                ...
                raise ValueError(
                    "Eurostat industrial production release PDF has too many unreadable pages"
                )
            text = " ".join(page_text)
        except (PdfReadError, ValueError) as exc:
            raise ValueError(
                "Eurostat industrial production release PDF text extraction failed"
            ) from exc
    else:
        raise ValueError(
            "Eurostat industrial production release value source format differs"
        )
    normalized = _SPACE_RE.sub(" ", text.replace("\u2212", "-")).strip()
    if not normalized:
        raise ValueError(
            "Eurostat industrial production release value source has no text"
        )
    return normalized
...
def _reference_period_from_publication(
    ...
    inferred = EUROSTAT_INDUSTRIAL_PRODUCTION_REFERENCE_PERIOD_INFERENCES.get(
        entry.product_code
    )
    if inferred is not None:
        return inferred, _release_lag(entry.release_date, inferred)
    raise ValueError(
        "Eurostat industrial production publication omits a qualified reference month for "
...
def _composition_from_publication(
    ...
    if labels and expected not in labels:
        raise ValueError(
            "Eurostat industrial production source-stated EA composition differs: "
            f"expected={expected}, observed={tuple(sorted(labels))}"
        )
    return expected, expected in labels


class _IndustrialProductionTableParser(HTMLParser):
    ...
    def __init__(self) -> None:
        ...
        self.table_measures: list[
            EurostatIndustrialProductionMeasure | None
        ] = []
        self._table: list[tuple[str, ...]] | None = None
        self._table_context = ""
        self._active_measure: EurostatIndustrialProductionMeasure | None = None
            ...
            if value:
                ...
                if (
                    ...
                    self._active_measure = (
                        EurostatIndustrialProductionMeasure.MONTH_OVER_MONTH
                    )
                elif (
                    ...
                    self._active_measure = (
                        EurostatIndustrialProductionMeasure.YEAR_OVER_YEAR
                    )
                elif "production indices for total industry" in normalized:
...
_INDUSTRIAL_PERIOD_CELL_RE = re.compile(
    r"^(?:(?P<name>[A-Za-z]{3,9})[-/ ](?P<name_year>\d{2,4})|"
    r"(?P<number>0?[1-9]|1[0-2])/(?P<number_year>\d{2,4})|"
    r"(?P<year>20\d{2})[-/](?P<year_month>0?[1-9]|1[0-2]))$"
)
_INDUSTRIAL_VALUE_CELL_RE = re.compile(r"^[+\-\u2212]?(?:\d+(?:\.\d+)?|\.\d+)")
_INDUSTRIAL_MONTHLY_MARKERS: Final = (
    "previous month",
    "monthly comparison",
    "month-on-month",
    "month on month",
)
_INDUSTRIAL_ANNUAL_MARKERS: Final = (
...
_INDUSTRIAL_PERIOD_TOKEN_RE = re.compile(
    r"(?<![A-Za-z0-9])(?:"
...
_INDUSTRIAL_PDF_ROW_RE = re.compile(
...
_INDUSTRIAL_PDF_NUMBER_RE = re.compile(
    r"(?<![A-Za-z0-9.])[+\-−]?(?:\d+(?:\.\d+)?|\.\d+)(?![A-Za-z0-9.])|:"
)


def _industrial_period_cell(value: str) -> str | None:
    token = _SPACE_RE.sub(" ", value).strip(" *\u00a0")
    match = _INDUSTRIAL_PERIOD_CELL_RE.fullmatch(token)
    if match is None:
        return None
    if match.group("name"):
        month = _INDUSTRIAL_MONTH_NAMES.get(match.group("name")[:3].casefold())
...
def _industrial_table_measure(
    rows: Sequence[Sequence[str]],
) -> EurostatIndustrialProductionMeasure | None:
    text = " ".join(cell.casefold() for row in rows[:4] for cell in row if cell)
    if any(marker in text for marker in _INDUSTRIAL_ANNUAL_MARKERS):
        return EurostatIndustrialProductionMeasure.YEAR_OVER_YEAR
    if any(marker in text for marker in _INDUSTRIAL_MONTHLY_MARKERS):
        return EurostatIndustrialProductionMeasure.MONTH_OVER_MONTH
    return None


def _industrial_economy_cell(
...
def _industrial_value_cell(value: str) -> str | None:
    token = _SPACE_RE.sub(" ", value).strip()
    if token in {"", ":", "-", ".."}:
        return None
    match = _INDUSTRIAL_VALUE_CELL_RE.match(token)
    if match is None:
        return None
    return match.group(0).replace("\u2212", "-")


def _industrial_html_table_values(
    snapshot: OfficialRawSnapshotV1,
    artifact: EurostatIndustrialProductionArtifactV1,
    reference_period: str,
) -> tuple[EurostatIndustrialProductionPublishedValueV1, ...]:
    ...
    try:
        ...
        source = snapshot.content.decode("cp1252")
    parser = _IndustrialProductionTableParser()
    parser.feed(source)
    parser.close()
    expected_ea_composition = _ea_composition_for_period(reference_period)
    values: dict[
        tuple[str, str, EurostatIndustrialProductionMeasure],
        EurostatIndustrialProductionPublishedValueV1,
    ] = {}
    for table_number, (context, declared_measure, rows) in enumerate(
        ...
        if "production indices for total industry" in table_header:
            continue
        measure = _industrial_table_measure(rows) or declared_measure
        if measure is None:
            continue
        header_index = -1
        ...
        for row_index, row in enumerate(rows):
            candidates = {
                ...
                if (period := _industrial_period_cell(cell)) is not None
            }
        ...
        for row in rows[header_index + 1 :]:
            economy = next(
                (
                    ...
                    for cell in row[:3]
                    if (
                        candidate := _industrial_economy_cell(
                            cell,
            ...
            for column, period in period_columns.items():
                if (
                    ...
                    continue
                lexical = _industrial_value_cell(row[column])
                if lexical is None:
                    continue
                item = EurostatIndustrialProductionPublishedValueV1(
                    ...
                    evidence_locator=(
                        f"industrial-production HTML table {table_number}; "
                ...
                if previous is not None and previous.value != item.value:
                    raise ValueError(
                        "Eurostat industrial production HTML tables conflict for "
...
def _industrial_pdf_page_values(
    page_text: str,
    artifact: EurostatIndustrialProductionArtifactV1,
    reference_period: str,
    page_number: int,
    ...
    measure = _industrial_table_measure((lines[:8],))
    if measure is None:
        return ()
    ...
    for line in lines:
        periods = tuple(
            period
            for match in _INDUSTRIAL_PERIOD_TOKEN_RE.finditer(line)
            if (period := _industrial_period_cell(match.group(0))) is not None
        )
        correction = (
            EUROSTAT_INDUSTRIAL_PRODUCTION_PERIOD_HEADER_CORRECTIONS.get(
        ...
        if not period_columns:
            continue
        row_match = _INDUSTRIAL_PDF_ROW_RE.match(line)
        if row_match is not None:
            economy = _industrial_economy_cell(
            ...
            r"^Total industry\d*\b", line, re.IGNORECASE
        ):
            ...
            continue
        pending_economy = None
        body = re.sub(r"^Total industry\d*\s*", "", body, flags=re.IGNORECASE)
        tokens = tuple(
            match.group(0).replace("\u2212", "-")
            for match in _INDUSTRIAL_PDF_NUMBER_RE.finditer(body)
        )
        ...
        for period, lexical in zip(period_columns, tokens, strict=True):
            ...
            item = EurostatIndustrialProductionPublishedValueV1(
                ...
                evidence_artifact_sha256=artifact.content_sha256,
                evidence_locator=(
                    f"industrial-production PDF page {page_number}; "
                    f"{economy} row; {period}; {measure.value}"
                ),
            )
            ...
            if previous is not None and previous.value != item.value:
                raise ValueError(
                    "Eurostat industrial production PDF tables conflict for "
                    f"{artifact.product_code}: {economy} {period} {measure.value}"
                )
...
def _industrial_pdf_table_values(
    snapshot: OfficialRawSnapshotV1,
    artifact: EurostatIndustrialProductionArtifactV1,
    reference_period: str,
) -> tuple[EurostatIndustrialProductionPublishedValueV1, ...]:
    ...
    try:
        ...
        raise ValueError(
            "Eurostat industrial production release PDF table extraction failed"
        ) from exc
    values: dict[
        tuple[str, str, EurostatIndustrialProductionMeasure],
        EurostatIndustrialProductionPublishedValueV1,
    ...
    for page_number, page in enumerate(reader.pages, start=1):
        try:
            ...
            if len(failed_pages) > 1:
                raise ValueError(
                    "Eurostat industrial production release PDF table has too many "
                    "unreadable pages"
                )
            continue
        if page_text.strip():
            readable_page_count += 1
        for item in _industrial_pdf_page_values(
            ...
            if previous is not None and previous.value != item.value:
                raise ValueError(
                    "Eurostat industrial production PDF pages conflict for "
    ...
    if not readable_page_count:
        raise ValueError(
            "Eurostat industrial production release PDF table has no readable text"
        )
    return tuple(
        sorted(
            values.values(),
...
def _industrial_table_published_values(
    snapshot: OfficialRawSnapshotV1,
    artifact: EurostatIndustrialProductionArtifactV1,
    reference_period: str,
) -> tuple[EurostatIndustrialProductionPublishedValueV1, ...]:
    values = _industrial_html_table_values(snapshot, artifact, reference_period)
    if values:
        return values
    return _industrial_pdf_table_values(snapshot, artifact, reference_period)


def _published_values(
    landing_snapshot: OfficialRawSnapshotV1,
    landing_artifact: EurostatIndustrialProductionArtifactV1,
    document_snapshot: OfficialRawSnapshotV1 | None,
    document_artifact: EurostatIndustrialProductionArtifactV1 | None,
    reference_period: str,
    headline: EurostatIndustrialProductionReleaseValueV1,
) -> tuple[EurostatIndustrialProductionPublishedValueV1, ...]:
    parsed: tuple[EurostatIndustrialProductionPublishedValueV1, ...] = ()
    for snapshot, artifact in (
        ...
        if snapshot is None or artifact is None:
            continue
        parsed = _industrial_table_published_values(
    ...
    headline_match = next(
        (
            ...
            and item.measure
            is EurostatIndustrialProductionMeasure.MONTH_OVER_MONTH
    ...
    if headline_match is None:
        values.append(
            EurostatIndustrialProductionPublishedValueV1(
                ...
                value_lexical=(
                    ...
                    is EurostatIndustrialProductionMovement.DOWN
        ...
        raise ValueError(
            "Eurostat industrial production headline and publication table differ for "
...
@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionReleaseV1:
    """One source-dated monthly industrial production release."""

    inventory_entry: EurostatIndustrialProductionInventoryEntryV1
    reference_period: str
    ...
    artifact: EurostatIndustrialProductionArtifactV1
    document_artifact: EurostatIndustrialProductionArtifactV1 | None
    headline_value: EurostatIndustrialProductionReleaseValueV1
    published_values: tuple[EurostatIndustrialProductionPublishedValueV1, ...]
    release_id: str = ""
    schema_version: str = EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASE_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                "Eurostat industrial production reference-period evidence differs"
            )
        ...
        if self.days_after_period_end != expected_lag:
            raise ValueError(
                "Eurostat industrial production release period or lag differs"
            )
        ...
        if composition != _ea_composition_for_period(reference):
            raise ValueError(
                "Eurostat industrial production EA composition differs"
            )
        ...
        expected_exception = (
            EUROSTAT_INDUSTRIAL_PRODUCTION_PAGE_DATE_OFFSET_EXCEPTIONS.get(
                self.inventory_entry.product_code
            )
        )
        if self.page_release_date_offset_days != offset or (
            ...
            raise ValueError(
                "Eurostat industrial production landing date offset differs for "
        ...
        if published_at is not None:
            ...
            if published_date.isoformat() != page_date:
                raise ValueError(
                    "Eurostat industrial production publication timestamp date differs"
                )
        links = tuple(
        ...
        if links != tuple(sorted(set(links))):
            raise ValueError(
                "Eurostat industrial production release document links are not canonical"
            )
        if len(links) > 1:
            raise ValueError(
                "Eurostat industrial production release has multiple English documents"
            )
        if any(
            ...
            raise ValueError(
                "Eurostat industrial production release document product code differs"
            )
        if (
            self.artifact.role
            is not EurostatIndustrialProductionArtifactRole.RELEASE_HTML
        ):
            raise ValueError(
                "Eurostat industrial production release artifact role differs"
            )
        if (
            ...
            raise ValueError(
                "Eurostat industrial production release artifact product code differs"
            )
        document = self.document_artifact
        if (document is None) != (not links):
            raise ValueError(
                "Eurostat industrial production release document artifact differs"
            )
        if document is not None and (
            ...
            raise ValueError(
                "Eurostat industrial production release document identity differs"
            )
        ...
        if (
            ...
            raise ValueError(
                "Eurostat industrial production release headline occurrence differs"
            )
        published_values = tuple(self.published_values)
        if not (
            1
            <= len(published_values)
            <= MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_PUBLISHED_VALUES_PER_RELEASE
        ):
            raise ValueError(
                "Eurostat industrial production published-value count differs"
            )
        keys = [item.key for item in published_values]
        if len(keys) != len(set(keys)):
            raise ValueError(
                "Eurostat industrial production release repeats a published value"
            )
        if list(published_values) != sorted(
            ...
            raise ValueError(
                "Eurostat industrial production published values are not canonical"
            )
        ...
        if any(
            ...
            raise ValueError(
                "Eurostat industrial production published-value evidence differs"
            )
        ...
        if (
            ...
            raise ValueError(
                "Eurostat industrial production headline published value differs"
            )
        object.__setattr__(self, "reference_period", reference)
        ...
        expected = _stable_id(
            "eurostat-industrial-production-release", self.identity_payload()
        )
        if self.release_id and self.release_id != expected:
            raise ValueError(
                "Eurostat industrial production release identity differs"
            )
    ...
    def from_dict(
        ...
        return cls(
            inventory_entry=EurostatIndustrialProductionInventoryEntryV1.from_dict(
            ...
            headline_value=EurostatIndustrialProductionReleaseValueV1.from_dict(
                _mapping(data.get("headline_value"), "headline_value")
            ),
            published_values=tuple(
                EurostatIndustrialProductionPublishedValueV1.from_dict(
...
def _published_revision_counts(
    releases: Sequence[EurostatIndustrialProductionReleaseV1],
) -> tuple[int, int]:
...
def _current_dataset_comparison_counts(
    releases: Sequence[EurostatIndustrialProductionReleaseV1],
    dataset: EurostatIndustrialProductionDatasetV1,
) -> tuple[int, int]:
    current = {
...
@dataclass(frozen=True, slots=True)
class EurostatIndustrialProductionArchiveManifestV1:
    """Compact, replayable receipt for the official industrial production corpus."""

    as_of_date: str
    registry_id: str
    source_id: str
    inventory: EurostatIndustrialProductionInventoryV1
    current_dataset: EurostatIndustrialProductionDatasetV1
    releases: tuple[EurostatIndustrialProductionReleaseV1, ...]
    ...
    manifest_id: str = ""
    schema_version: str = EUROSTAT_INDUSTRIAL_PRODUCTION_MANIFEST_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat industrial production manifest schema"
            )
        as_of = _iso_date(self.as_of_date, "as_of_date")
        if as_of != EUROSTAT_INDUSTRIAL_PRODUCTION_AS_OF_DATE:
            raise ValueError(
                "Eurostat industrial production archive boundary differs"
            )
        ...
        if not registry_id.startswith("official-source-registry:sha256:"):
            raise ValueError(
                "Eurostat industrial production registry identity is invalid"
            )
        if not source_id.startswith("official-source:sha256:"):
            raise ValueError(
                "Eurostat industrial production source identity is invalid"
            )
        releases = tuple(self.releases)
        if len(releases) != EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASE_COUNT:
            raise ValueError(
                "Eurostat industrial production manifest release count differs"
            )
        if list(releases) != sorted(
            ...
            raise ValueError(
                "Eurostat industrial production releases are not canonical"
            )
        if [item.inventory_entry for item in releases] != list(
            self.inventory.entries
        ):
            raise ValueError(
                "Eurostat industrial production release and inventory entries differ"
            )
        if any(
            item.headline_value
            ...
            raise ValueError(
                "Eurostat industrial production release headline evidence differs"
            )
        occurrences = [item.occurrence_key for item in releases]
        if len(occurrences) != len(set(occurrences)):
            raise ValueError(
                "Eurostat industrial production releases repeat an occurrence"
            )
        periods = [item.reference_period for item in releases]
        if len(periods) != len(set(periods)):
            raise ValueError(
                "Eurostat industrial production releases repeat a month"
            )
        if (
            min(periods)
            ...
            raise ValueError(
                "Eurostat industrial production reference range differs"
            )
        expected_release_periods = tuple(
            ...
            if item
            not in EUROSTAT_INDUSTRIAL_PRODUCTION_INTERNAL_GAP_REFERENCE_PERIODS
        )
        if tuple(periods) != expected_release_periods:
            raise ValueError(
                "Eurostat industrial production reference-month lineage differs"
            )
        if any(item.inventory_entry.release_date > as_of for item in releases):
            raise ValueError(
                "Eurostat industrial production release exceeds archive boundary"
            )
        if self.current_dataset.updated_at[:10] > as_of:
            raise ValueError(
                "Eurostat industrial production dataset exceeds archive boundary"
            )
        expected_gaps = (
            *_month_range(
                self.current_dataset.time_start,
                _month_shift(
                    EUROSTAT_INDUSTRIAL_PRODUCTION_FIRST_REFERENCE_PERIOD, -1
                ),
            ),
        ...
        if gaps != expected_gaps:
            raise ValueError(
                "Eurostat industrial production searchable-release gap inventory differs"
            )
        for name, maximum in (
            (
                "reference_period_inferred_count",
                MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASES,
            ),
            (
        ...
        if self.reference_period_inferred_count != expected_inferred_references:
            raise ValueError(
                "Eurostat industrial production inferred-reference count differs"
            )
        if self.composition_source_stated_count != sum(
            item.composition_source_stated for item in releases
        ):
            raise ValueError(
                "Eurostat industrial production composition count differs"
            )
        ...
        if self.page_date_offset_count != expected_offsets:
            raise ValueError(
                "Eurostat industrial production page-date offset count differs"
            )
        ...
        if self.exact_publication_time_count != expected_exact_times:
            raise ValueError(
                "Eurostat industrial production exact publication-time count differs"
            )
        ...
        if self.release_document_count != len(document_artifacts):
            raise ValueError(
                "Eurostat industrial production release-document count differs"
            )
        ...
        if revision_counts != (
            ...
            raise ValueError(
                "Eurostat industrial production revision counts differ"
            )
        ...
        if dataset_counts != (
            ...
            raise ValueError(
                "Eurostat industrial production dataset comparison differs"
            )
        artifacts = (
        ...
        counts = (
            _bounded_int(
                ...
                "raw_artifact_count",
                MAX_EUROSTAT_INDUSTRIAL_PRODUCTION_ARTIFACTS,
            ),
            _bounded_int(
        ...
        if counts != (
            ...
            raise ValueError(
                "Eurostat industrial production artifact totals differ"
            )
        ...
        expected = _stable_id(
            "eurostat-industrial-production-archive-manifest",
            self.identity_payload(),
        )
        if self.manifest_id and self.manifest_id != expected:
            raise ValueError(
                "Eurostat industrial production manifest identity differs"
            )
    ...
    def from_dict(
        ...
        return cls(
            ...
            inventory=EurostatIndustrialProductionInventoryV1.from_dict(
                _mapping(data.get("inventory"), "inventory")
            ),
            current_dataset=EurostatIndustrialProductionDatasetV1.from_dict(
                _mapping(data.get("current_dataset"), "current_dataset")
            ),
            releases=tuple(
                EurostatIndustrialProductionReleaseV1.from_dict(
    ...
    def from_json(
        ...
        try:
            ...
            raise ValueError(
                "Eurostat industrial production manifest is invalid JSON"
            ) from exc
        return cls.from_dict(_mapping(data, "manifest"))


def build_eurostat_industrial_production_archive_manifest(
    registry: OfficialSourceRegistryV1,
    ...
    *,
    ...
    """Build the industrial production receipt from exact retained Eurostat bytes."""
    source = registry.source(EUROSTAT_INDUSTRIAL_PRODUCTION_SOURCE_KEY)
    if (
        source.parser_id != EUROSTAT_INDUSTRIAL_PRODUCTION_PARSER_ID
        or source.parser_version
        != EUROSTAT_INDUSTRIAL_PRODUCTION_PARSER_VERSION
    ):
        raise ValueError(
            "Eurostat industrial production registry parser identity differs"
        )
    ...
    if any(
        ...
        raise ValueError(
            "Eurostat industrial production snapshot source identity differs"
        )
    inventory = parse_eurostat_industrial_production_inventory(
        search_values, as_of_date=as_of_date
    )
    dataset = parse_eurostat_industrial_production_dataset(dataset_snapshot)
    snapshots = release_values
    if len(snapshots) != EUROSTAT_INDUSTRIAL_PRODUCTION_RELEASE_COUNT:
        raise ValueError(
            "Eurostat industrial production release snapshot count differs"
        )
    by_code: dict[str, OfficialRawSnapshotV1] = {}
    for snapshot in snapshots:
        code = _product_code(snapshot.request.uri)
        if code in by_code:
            raise ValueError(
                "Eurostat industrial production release corpus repeats a product code"
            )
        by_code[code] = snapshot
    if set(by_code) != {item.product_code for item in inventory.entries}:
        raise ValueError(
            "Eurostat industrial production release corpus is incomplete"
        )
    if len(document_values) != EUROSTAT_INDUSTRIAL_PRODUCTION_DOCUMENT_COUNT:
        raise ValueError(
            "Eurostat industrial production document snapshot count differs"
        )
    ...
    for snapshot in document_values:
        if snapshot.request.uri in documents_by_uri:
            raise ValueError(
                "Eurostat industrial production document corpus repeats a URI"
            )
    ...
    parsed: list[
        tuple[
            EurostatIndustrialProductionInventoryEntryV1,
            EurostatIndustrialProductionArtifactV1,
            EurostatIndustrialProductionArtifactV1 | None,
            str,
            str | None,
            str,
            int,
    ...
    for entry in inventory.entries:
        ...
        if snapshot.request.uri != entry.source_uri:
            raise ValueError(
                "Eurostat industrial production release request identity differs"
            )
        artifact = _artifact_from_snapshot(
            snapshot,
            EurostatIndustrialProductionArtifactRole.RELEASE_HTML,
        ...
        document_artifact: EurostatIndustrialProductionArtifactV1 | None = None
        ...
        if document_uris:
            ...
            try:
                ...
                raise ValueError(
                    "Eurostat industrial production document corpus is incomplete"
                ) from exc
            ...
            if document_role is None:
                raise ValueError(
                    "Eurostat industrial production document format differs"
                )
    ...
    if used_document_uris != set(documents_by_uri):
        raise ValueError(
            "Eurostat industrial production document corpus has unexpected artifacts"
        )
    releases: list[EurostatIndustrialProductionReleaseV1] = []
    for (
        entry,
        artifact,
        document_artifact,
        ...
        if lag != expected_lag:
            raise ValueError(
                "Eurostat industrial production release lag changed during parsing"
            )
        ...
        releases.append(
            EurostatIndustrialProductionReleaseV1(
                ...
                reference_period_source_stated=(
                    entry.product_code
                    not in EUROSTAT_INDUSTRIAL_PRODUCTION_REFERENCE_PERIOD_INFERENCES
                ),
    ...
    return EurostatIndustrialProductionArchiveManifestV1(
...
def replay_eurostat_industrial_production_archive(
    registry: OfficialSourceRegistryV1,
    search_snapshots: Sequence[OfficialRawSnapshotV1],
    dataset_snapshot: OfficialRawSnapshotV1,
    release_snapshots: Sequence[OfficialRawSnapshotV1],
    document_snapshots: Sequence[OfficialRawSnapshotV1],
    expected_manifest: EurostatIndustrialProductionArchiveManifestV1,
) -> EurostatIndustrialProductionArchiveManifestV1:
    """Rebuild the industrial production receipt and require byte-for-byte identity."""
    rebuilt = build_eurostat_industrial_production_archive_manifest(
        registry,
        search_snapshots,
        dataset_snapshot,
        release_snapshots,
    ...
    if rebuilt.to_json() != expected_manifest.to_json():
        raise ValueError(
            "Eurostat industrial production replay differs from packaged manifest"
        )
    return rebuilt


def packaged_eurostat_industrial_production_manifest_path() -> Path:
    """Return the installed industrial production archive-manifest path."""
    return (
        Path(__file__).resolve().parent
        / "assets"
        / "eurostat_industrial_production_archive_v1.json"
    )


def load_packaged_eurostat_industrial_production_archive_manifest() -> (
    EurostatIndustrialProductionArchiveManifestV1
):
    """Load and validate the installed industrial production archive manifest."""
    path = packaged_eurostat_industrial_production_manifest_path()
    if path.stat().st_size > 16 * 1024 * 1024:
        raise ValueError(
            "packaged Eurostat industrial production manifest exceeds size bound"
        )
    manifest = EurostatIndustrialProductionArchiveManifestV1.from_json(
    ...
    if (
        ...
        raise ValueError(
            "packaged Eurostat industrial production registry binding differs"
        )
    return manifest
