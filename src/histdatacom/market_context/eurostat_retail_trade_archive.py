"""Deterministic qualification of Eurostat monthly retail trade releases.
...
``sts_trtu_m`` JSON-stat table is retained only as a revised-series cross-check;
...
EUROSTAT_RETAIL_TRADE_SOURCE_KEY: Final = "ea.eurostat.retail-trade"
EUROSTAT_RETAIL_TRADE_PROGRAM_KEY: Final = "ea.eurostat.monthly-retail-trade"
EUROSTAT_RETAIL_TRADE_MEASURE_KEY: Final = "retail-trade-volume-total"
EUROSTAT_RETAIL_TRADE_PARSER_ID: Final = "official.eurostat-retail-trade.v1"
EUROSTAT_RETAIL_TRADE_PARSER_VERSION: Final = "1"
EUROSTAT_RETAIL_TRADE_SOURCE_TIMEZONE: Final = "Europe/Brussels"
EUROSTAT_RETAIL_TRADE_AS_OF_DATE: Final = "2026-09-18"
EUROSTAT_RETAIL_TRADE_FIRST_SEARCH_RELEASE_DATE: Final = "2000-04-12"
EUROSTAT_RETAIL_TRADE_LATEST_PACKAGED_RELEASE_DATE: Final = "2026-09-04"
EUROSTAT_RETAIL_TRADE_FIRST_REFERENCE_PERIOD: Final = "2000-01"
EUROSTAT_RETAIL_TRADE_LATEST_REFERENCE_PERIOD: Final = "2026-07"
EUROSTAT_RETAIL_TRADE_MONTHLY_HEADLINE_FIRST_REFERENCE_PERIOD: Final = "2007-07"
EUROSTAT_RETAIL_TRADE_INTERNAL_GAP_REFERENCE_PERIODS: Final = (
    "2000-02",
    "2000-03",
    "2000-04",
    "2000-05",
...
EUROSTAT_RETAIL_TRADE_PAGE_DATE_OFFSET_EXCEPTIONS: Final = {
    "4-04052007-ap": 3,
    "4-19032009-bp": -6,
}
EUROSTAT_RETAIL_TRADE_REFERENCE_PERIOD_INFERENCES: Final = {}
EUROSTAT_RETAIL_TRADE_PERIOD_HEADER_CORRECTIONS: Final = {}
EUROSTAT_RETAIL_TRADE_SEARCH_PAGE_COUNT: Final = 5
EUROSTAT_RETAIL_TRADE_SEARCH_RESULT_COUNT: Final = 417
EUROSTAT_RETAIL_TRADE_SEARCH_UNIQUE_URI_COUNT: Final = 417
EUROSTAT_RETAIL_TRADE_TITLE_MATCH_COUNT: Final = 298
EUROSTAT_RETAIL_TRADE_PREFILTER_COUNT: Final = 298
EUROSTAT_RETAIL_TRADE_RELEASE_COUNT: Final = 298
EUROSTAT_RETAIL_TRADE_DOCUMENT_COUNT: Final = 267

EUROSTAT_RETAIL_TRADE_SEARCH_URI: Final = "https://ec.europa.eu/eurostat/search"
EUROSTAT_RETAIL_TRADE_PRODUCT_URI_TEMPLATE: Final = (
    "https://ec.europa.eu/eurostat/product?code={product_code}"
)
EUROSTAT_RETAIL_TRADE_DATASET_URI: Final = (
    "https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/"
    "sts_trtu_m?lang=en&indic_bt=VOL_SLS&nace_r2=G47&"
    "s_adj=SCA&s_adj=CA&unit=PCH_PRE&unit=PCH_SM&"
    "geo=EA21&geo=DE&geo=FR&sinceTimePeriod=2000-01"
)

EUROSTAT_RETAIL_TRADE_ARTIFACT_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-artifact.v1"
)
EUROSTAT_RETAIL_TRADE_INVENTORY_ENTRY_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-inventory-entry.v1"
)
EUROSTAT_RETAIL_TRADE_EXCLUSION_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-exclusion.v1"
)
EUROSTAT_RETAIL_TRADE_INVENTORY_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-inventory.v1"
)
EUROSTAT_RETAIL_TRADE_OBSERVATION_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-observation.v1"
)
EUROSTAT_RETAIL_TRADE_DATASET_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-dataset.v1"
)
EUROSTAT_RETAIL_TRADE_RELEASE_VALUE_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-release-value.v1"
)
EUROSTAT_RETAIL_TRADE_PUBLISHED_VALUE_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-published-value.v1"
)
EUROSTAT_RETAIL_TRADE_RELEASE_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-release.v1"
)
EUROSTAT_RETAIL_TRADE_MANIFEST_SCHEMA_VERSION: Final = (
    "histdatacom.eurostat-retail-trade-archive-manifest.v1"
)

MAX_EUROSTAT_RETAIL_TRADE_INDEX_PAGES: Final = 32
MAX_EUROSTAT_RETAIL_TRADE_SEARCH_ENTRIES: Final = 2_048
MAX_EUROSTAT_RETAIL_TRADE_RELEASES: Final = 512
MAX_EUROSTAT_RETAIL_TRADE_OBSERVATIONS: Final = 4_096
MAX_EUROSTAT_RETAIL_TRADE_ARTIFACTS: Final = 2_048
MAX_EUROSTAT_RETAIL_TRADE_TOTAL_BYTES: Final = 1_024_000_000
MAX_EUROSTAT_RETAIL_TRADE_TITLE_CHARS: Final = 512
MAX_EUROSTAT_RETAIL_TRADE_PUBLISHED_VALUES_PER_RELEASE: Final = 96
...
_PRODUCT_RELEASE_DATE_OVERRIDES: Final = {
    # Selected legacy code contains a source typo; Atom dates it 4 August 2004.
...
def _product_code(uri_or_code: object) -> str:
    ...
    if parsed.scheme and parsed.path == "/eurostat/product":
        ...
        if len(candidates) != 1:
            raise ValueError(
                "Eurostat retail trade product URI has no unique product code"
            )
        token = candidates[0]
    else:
    ...
    if match is None:
        raise ValueError("Eurostat retail trade product code is invalid")
...
def _inventory_prefilter(title: str) -> bool:
    ...
    return normalized.startswith("volume of retail trade ")
...
def _exclusion_reason(title: str) -> str:
    normalized = _required_text(title, "title").casefold()
    if "international trade" in normalized or "trade in goods" in normalized:
        return "external-trade release, not retail sales volume"
    if "labour" in normalized or "job vacancy" in normalized:
        return "labour-market release, not retail sales volume"
    if "industrial" in normalized or "production" in normalized:
        return "production release, not retail sales volume"
    return "broad search-summary match outside the monthly headline lineage"


class EurostatRetailTradeArtifactRole(str, Enum):
    ...
    @classmethod
    def from_value(
        cls, value: str | EurostatRetailTradeArtifactRole
    ) -> EurostatRetailTradeArtifactRole:
        try:
            return value if isinstance(value, cls) else cls(value)
        except ValueError as exc:
            raise ValueError(
                "unsupported Eurostat retail trade artifact role"
            ) from exc


class EurostatRetailTradeMeasure(str, Enum):
    """Qualified retail trade statistic carried by this archive."""
    ...
    @classmethod
    def from_value(
        cls, value: str | EurostatRetailTradeMeasure
    ) -> EurostatRetailTradeMeasure:
        try:
            return value if isinstance(value, cls) else cls(value)
        except ValueError as exc:
            raise ValueError(
                "unsupported Eurostat retail trade measure"
            ) from exc


@dataclass(frozen=True, slots=True)
class EurostatRetailTradeArtifactV1:
    """Content-addressed official retail trade evidence artifact."""

    role: EurostatRetailTradeArtifactRole
    request_uri: str
    resolved_uri: str
    ...
    schema_version: str = EUROSTAT_RETAIL_TRADE_ARTIFACT_SCHEMA_VERSION

    def __post_init__(self) -> None:
        if self.schema_version != EUROSTAT_RETAIL_TRADE_ARTIFACT_SCHEMA_VERSION:
            raise ValueError(
                "unsupported Eurostat retail trade artifact schema"
            )
        role = EurostatRetailTradeArtifactRole.from_value(self.role)
        ...
        expected_format = {
            EurostatRetailTradeArtifactRole.SEARCH_INDEX: OfficialSourceFormat.ATOM,
            EurostatRetailTradeArtifactRole.CURRENT_DATASET: OfficialSourceFormat.JSON_STAT,
            EurostatRetailTradeArtifactRole.RELEASE_HTML: OfficialSourceFormat.HTML,
            EurostatRetailTradeArtifactRole.RELEASE_DOCUMENT_HTML: OfficialSourceFormat.HTML,
            EurostatRetailTradeArtifactRole.RELEASE_DOCUMENT_PDF: OfficialSourceFormat.PDF,
        }[role]
        if source_format is not expected_format:
            raise ValueError(
                "Eurostat retail trade artifact role and format differ"
            )
        length = _bounded_int(
            self.content_length,
            "content_length",
            MAX_EUROSTAT_RETAIL_TRADE_TOTAL_BYTES,
        )
        if length < 1:
            raise ValueError("Eurostat retail trade artifact is empty")
        page_number = self.page_number
        product_code = self.product_code
        if role is EurostatRetailTradeArtifactRole.SEARCH_INDEX:
            if page_number is None:
                raise ValueError(
                    "Eurostat retail trade search artifact needs a page number"
                )
            _bounded_int(
                page_number,
                "page_number",
                MAX_EUROSTAT_RETAIL_TRADE_INDEX_PAGES,
            )
            if product_code is not None:
                raise ValueError(
                    "Eurostat retail trade search artifact has a product code"
                )
        elif role is EurostatRetailTradeArtifactRole.CURRENT_DATASET:
            if page_number is not None or product_code is not None:
                raise ValueError(
                    "Eurostat retail trade dataset artifact has release metadata"
                )
        else:
            if page_number is not None or product_code is None:
                raise ValueError(
                    "Eurostat retail trade release artifact metadata differs"
                )
    ...
    def from_dict(
        ...
        return cls(
            role=EurostatRetailTradeArtifactRole.from_value(
                str(data.get("role", ""))
            ),
...
@dataclass(frozen=True, slots=True)
class EurostatRetailTradeInventoryEntryV1:
    ...
    page_number: int
    entry_id: str = ""
    schema_version: str = EUROSTAT_RETAIL_TRADE_INVENTORY_ENTRY_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat retail trade inventory-entry schema"
            )
        ...
        if release_date != _product_release_date(product_code):
            raise ValueError(
                "Eurostat retail trade release date differs from product code"
            )
        title = _required_text(
            self.source_title,
            "source_title",
            maximum=MAX_EUROSTAT_RETAIL_TRADE_TITLE_CHARS,
        )
        if not _inventory_selected(title, product_code):
            raise ValueError(
                "Eurostat retail trade inventory entry is outside selection"
            )
        ...
        if (
            ...
            raise ValueError("Eurostat retail trade source URI route differs")
        if _product_code(source_uri) != product_code:
            raise ValueError(
                "Eurostat retail trade source URI and product code differ"
            )
        page = _bounded_int(
            self.page_number,
            "page_number",
        ...
        if page < 1:
            raise ValueError(
                "Eurostat retail trade page number must be positive"
            )
        object.__setattr__(self, "product_code", product_code)
        ...
        expected = _stable_id(
            "eurostat-retail-trade-inventory-entry",
            self.identity_payload(),
        )
        if self.entry_id and self.entry_id != expected:
            raise ValueError(
                "Eurostat retail trade inventory-entry identity differs"
            )
...
@dataclass(frozen=True, slots=True)
class EurostatRetailTradeIndexExclusionV1:
    ...
    reason: str
    exclusion_id: str = ""
    schema_version: str = EUROSTAT_RETAIL_TRADE_EXCLUSION_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat retail trade exclusion schema"
            )
        product_code = _product_code(self.product_code)
        release_date = _iso_date(self.release_date, "release_date")
        if release_date != _product_release_date(product_code):
            raise ValueError("Eurostat retail trade exclusion date differs")
        title = _required_text(
            self.source_title,
            "source_title",
            maximum=MAX_EUROSTAT_RETAIL_TRADE_TITLE_CHARS,
        ...
        if _inventory_selected(title, product_code):
            raise ValueError(
                "Eurostat retail trade exclusion matches selection"
            )
        source_uri = _https_uri(self.source_uri, "source_uri")
        if _product_code(source_uri) != product_code:
            raise ValueError("Eurostat retail trade exclusion URI differs")
        reason = _required_text(self.reason, "reason")
        if reason != _exclusion_reason(title):
            raise ValueError("Eurostat retail trade exclusion reason differs")
        object.__setattr__(self, "product_code", product_code)
        object.__setattr__(self, "release_date", release_date)
        object.__setattr__(self, "source_title", title)
        object.__setattr__(self, "source_uri", source_uri)
        ...
        expected = _stable_id(
            "eurostat-retail-trade-exclusion", self.identity_payload()
        )
        if self.exclusion_id and self.exclusion_id != expected:
            raise ValueError("Eurostat retail trade exclusion identity differs")
...
@dataclass(frozen=True, slots=True)
class EurostatRetailTradeInventoryV1:
    """Hash-bound official search inventory and deterministic selection."""

    artifacts: tuple[EurostatRetailTradeArtifactV1, ...]
    entries: tuple[EurostatRetailTradeInventoryEntryV1, ...]
    exclusions: tuple[EurostatRetailTradeIndexExclusionV1, ...]
    query_result_count: int
    unique_uri_count: int
    title_match_count: int
    prefilter_count: int
    inventory_id: str = ""
    schema_version: str = EUROSTAT_RETAIL_TRADE_INVENTORY_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat retail trade inventory schema"
            )
        artifacts = tuple(self.artifacts)
        entries = tuple(self.entries)
        exclusions = tuple(self.exclusions)
        if len(artifacts) != EUROSTAT_RETAIL_TRADE_SEARCH_PAGE_COUNT:
            raise ValueError("Eurostat retail trade search page count differs")
        if any(
            item.role is not EurostatRetailTradeArtifactRole.SEARCH_INDEX
            for item in artifacts
        ):
            raise ValueError(
                "Eurostat retail trade inventory artifact role differs"
            )
        if [item.page_number for item in artifacts] != list(
            range(1, EUROSTAT_RETAIL_TRADE_SEARCH_PAGE_COUNT + 1)
        ):
            raise ValueError(
                "Eurostat retail trade search pages are not contiguous"
            )
        if any(
            ...
            raise ValueError(
                "Eurostat retail trade search artifact identity differs"
            )
        if len(entries) != EUROSTAT_RETAIL_TRADE_RELEASE_COUNT:
            raise ValueError(
                "Eurostat retail trade selected release count differs"
            )
        codes = [item.product_code for item in entries]
        if len(codes) != len(set(codes)):
            raise ValueError(
                "Eurostat retail trade inventory repeats a product code"
            )
        if list(entries) != sorted(
            entries, key=lambda item: (item.release_date, item.product_code)
        ):
            raise ValueError(
                "Eurostat retail trade inventory entries are not canonical"
            )
        if (
            ...
            raise ValueError(
                "Eurostat retail trade first searchable release differs"
            )
        if (
            ...
            raise ValueError(
                "Eurostat retail trade latest searchable release differs"
            )
        if len(exclusions) != (
            EUROSTAT_RETAIL_TRADE_SEARCH_UNIQUE_URI_COUNT
            - EUROSTAT_RETAIL_TRADE_RELEASE_COUNT
        ):
            raise ValueError("Eurostat retail trade exclusion count differs")
        exclusion_codes = [item.product_code for item in exclusions]
        if len(exclusion_codes) != len(set(exclusion_codes)):
            raise ValueError(
                "Eurostat retail trade exclusions repeat a product"
            )
        if list(exclusions) != sorted(
            exclusions, key=lambda item: (item.release_date, item.product_code)
        ):
            raise ValueError(
                "Eurostat retail trade exclusions are not canonical"
            )
        if set(codes) & set(exclusion_codes):
            raise ValueError(
                "Eurostat retail trade selection includes an exclusion"
            )
        expected_counts = (
            EUROSTAT_RETAIL_TRADE_SEARCH_RESULT_COUNT,
            EUROSTAT_RETAIL_TRADE_SEARCH_UNIQUE_URI_COUNT,
            EUROSTAT_RETAIL_TRADE_TITLE_MATCH_COUNT,
            EUROSTAT_RETAIL_TRADE_PREFILTER_COUNT,
        )
        observed_counts = (
            _bounded_int(
                self.query_result_count,
                "query_result_count",
                MAX_EUROSTAT_RETAIL_TRADE_SEARCH_ENTRIES,
            ),
            _bounded_int(
        ...
        if observed_counts != expected_counts:
            raise ValueError(
                "Eurostat retail trade search inventory counts differ"
            )
        object.__setattr__(self, "artifacts", artifacts)
        object.__setattr__(self, "entries", entries)
        object.__setattr__(self, "exclusions", exclusions)
        expected = _stable_id(
            "eurostat-retail-trade-inventory", self.identity_payload()
        )
        if self.inventory_id and self.inventory_id != expected:
            raise ValueError("Eurostat retail trade inventory identity differs")
...
@dataclass(frozen=True, slots=True)
class EurostatRetailTradeObservationV1:
    """One current-revised JSON-stat retail-trade observation."""

    economy_code: str
    reference_period: str
    measure: EurostatRetailTradeMeasure
    value_lexical: str
    value: float
    status: str | None
    flat_index: int
    observation_id: str = ""
    schema_version: str = EUROSTAT_RETAIL_TRADE_OBSERVATION_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        ...
        expected = _stable_id(
            "eurostat-retail-trade-observation",
            self.identity_payload(),
        )
        if self.observation_id and self.observation_id != expected:
            raise ValueError(
                "Eurostat retail trade observation identity differs"
            )
    ...
    def from_dict(
        ...
        return cls(
            ...
            measure=EurostatRetailTradeMeasure.from_value(
...
@dataclass(frozen=True, slots=True)
class EurostatRetailTradeDatasetV1:
    """Exact current-revised ``sts_trtu_m`` cross-check response."""

    label: str
    updated_at: str
    time_start: str
    time_end: str
    artifact: EurostatRetailTradeArtifactV1
    observations: tuple[EurostatRetailTradeObservationV1, ...]
    dataset_id: str = "sts_trtu_m"
    dataset_receipt_id: str = ""
    schema_version: str = EUROSTAT_RETAIL_TRADE_DATASET_SCHEMA_VERSION

    def __post_init__(self) -> None:
        if self.schema_version != EUROSTAT_RETAIL_TRADE_DATASET_SCHEMA_VERSION:
            raise ValueError("unsupported Eurostat retail trade dataset schema")
        if self.dataset_id != "sts_trtu_m":
            raise ValueError("unsupported Eurostat retail trade dataset")
        ...
        if time_end < time_start:
            raise ValueError(
                "Eurostat retail trade dataset time range is reversed"
            )
        if (
            self.artifact.role
            is not EurostatRetailTradeArtifactRole.CURRENT_DATASET
        ):
            raise ValueError(
                "Eurostat retail trade dataset artifact role differs"
            )
        if (
            self.artifact.request_uri != EUROSTAT_RETAIL_TRADE_DATASET_URI
            or self.artifact.resolved_uri != EUROSTAT_RETAIL_TRADE_DATASET_URI
        ):
            raise ValueError(
                "Eurostat retail trade dataset artifact identity differs"
            )
        observations = tuple(self.observations)
        if not 1 <= len(observations) <= MAX_EUROSTAT_RETAIL_TRADE_OBSERVATIONS:
            raise ValueError(
                "Eurostat retail trade observation count is invalid"
            )
        keys = [item.key for item in observations]
        if len(keys) != len(set(keys)):
            raise ValueError(
                "Eurostat retail trade dataset repeats an observation"
            )
        ...
        expected_missing = {
            (
                EurostatRetailTradeMeasure.MONTH_OVER_MONTH.value,
                "EA21",
                "2000-01",
            )
        }
        expected_missing.update(
            (
                EurostatRetailTradeMeasure.YEAR_OVER_YEAR.value,
                "EA21",
                f"2000-{month:02d}",
            )
            for month in range(1, 13)
        )
        expected_missing &= expected_keys
        if missing_keys != expected_missing or set(keys) - expected_keys:
            raise ValueError(
                "Eurostat retail trade dataset cube coverage differs"
            )
        period_positions = {
            period: position for position, period in enumerate(periods)
        }
        measure_coordinates = {
            EurostatRetailTradeMeasure.MONTH_OVER_MONTH: (1, 0),
            EurostatRetailTradeMeasure.YEAR_OVER_YEAR: (0, 1),
        }
        if any(
            item.flat_index
            != (
            ...
            raise ValueError(
                "Eurostat retail trade observation coordinate differs"
            )
        if list(observations) != sorted(
            observations,
            key=lambda item: (
                list(EurostatRetailTradeMeasure).index(item.measure),
                _ECONOMY_ORDER[item.economy_code],
                item.reference_period,
            ),
        ):
            raise ValueError(
                "Eurostat retail trade observations are not canonical"
            )
        if (
            ...
            raise ValueError("Eurostat retail trade dataset time range differs")
        ...
        expected = _stable_id(
            "eurostat-retail-trade-dataset", self.identity_payload()
        )
        if self.dataset_receipt_id and self.dataset_receipt_id != expected:
            raise ValueError("Eurostat retail trade dataset identity differs")
    ...
    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> EurostatRetailTradeDatasetV1:
        return cls(
            label=str(data.get("label", "")),
            updated_at=str(data.get("updated_at", "")),
            time_start=str(data.get("time_start", "")),
            time_end=str(data.get("time_end", "")),
            artifact=EurostatRetailTradeArtifactV1.from_dict(
                _mapping(data.get("artifact"), "artifact")
            ),
            observations=tuple(
                EurostatRetailTradeObservationV1.from_dict(
...
def _search_uri(page_number: int) -> str:
    ...
    if page < 1:
        raise ValueError("Eurostat retail trade search page must be positive")
    params = {
        "p_p_id": (
...
def _request(
    ...
    *,
    ...
    source = registry.source(EUROSTAT_RETAIL_TRADE_SOURCE_KEY)
...
def build_eurostat_retail_trade_search_requests(
    registry: OfficialSourceRegistryV1,
    *,
    page_count: int = EUROSTAT_RETAIL_TRADE_SEARCH_PAGE_COUNT,
) -> tuple[OfficialSourceRequestV1, ...]:
    """Build the bounded official Atom inventory requests."""
    count = _bounded_int(
        page_count, "page_count", MAX_EUROSTAT_RETAIL_TRADE_INDEX_PAGES
    )
    if count < 1:
        raise ValueError("Eurostat retail trade search needs at least one page")
    return tuple(
        _request(
            registry,
            _search_uri(page),
...
def build_eurostat_retail_trade_dataset_request(
    registry: OfficialSourceRegistryV1,
) -> OfficialSourceRequestV1:
    """Build the exact current-revised retail trade cross-check request."""
    return _request(
        registry,
...
def build_eurostat_retail_trade_release_requests(
    registry: OfficialSourceRegistryV1,
    inventory: EurostatRetailTradeInventoryV1,
) -> tuple[OfficialSourceRequestV1, ...]:
    """Build one primary request for every selected retail trade product."""
    if not isinstance(inventory, EurostatRetailTradeInventoryV1):
        raise TypeError(
            "Eurostat retail trade release requests require a v1 inventory"
        )
    return tuple(
...
def _document_source_format(uri: str) -> OfficialSourceFormat:
    ...
    raise ValueError("Eurostat retail trade release document format differs")


def build_eurostat_retail_trade_document_requests(
    registry: OfficialSourceRegistryV1,
    inventory: EurostatRetailTradeInventoryV1,
    ...
    if not isinstance(inventory, EurostatRetailTradeInventoryV1):
        raise TypeError(
            "Eurostat retail trade document requests require a v1 inventory"
        )
    entries = {item.product_code: item for item in inventory.entries}
    snapshots = tuple(release_snapshots)
    if len(snapshots) != EUROSTAT_RETAIL_TRADE_RELEASE_COUNT:
        raise ValueError("Eurostat retail trade landing snapshot count differs")
    by_code: dict[str, OfficialRawSnapshotV1] = {}
    for snapshot in snapshots:
        code = _product_code(snapshot.request.uri)
        if code in by_code:
            raise ValueError(
                "Eurostat retail trade landing corpus repeats a product code"
            )
        by_code[code] = snapshot
    if set(by_code) != set(entries):
        raise ValueError("Eurostat retail trade landing corpus is incomplete")
    requests: list[OfficialSourceRequestV1] = []
    for code, entry in entries.items():
        snapshot = by_code[code]
        if snapshot.request.uri != entry.source_uri:
            raise ValueError(
                "Eurostat retail trade landing request identity differs"
            )
    ...
    if (
        ...
        and len(requests) != EUROSTAT_RETAIL_TRADE_DOCUMENT_COUNT
    ):
        raise ValueError("Eurostat retail trade English document count differs")
...
def _artifact_from_snapshot(
    ...
    *,
    ...
    if not isinstance(snapshot, OfficialRawSnapshotV1):
        raise TypeError("Eurostat retail trade artifact requires a v1 snapshot")
    if (
        snapshot.request.source_key != EUROSTAT_RETAIL_TRADE_SOURCE_KEY
        or snapshot.request.parser_id != EUROSTAT_RETAIL_TRADE_PARSER_ID
        or snapshot.request.parser_version
        != EUROSTAT_RETAIL_TRADE_PARSER_VERSION
    ):
        raise ValueError(
            "Eurostat retail trade snapshot source or parser differs"
        )
    if snapshot.request.method is not OfficialRequestMethod.GET:
        raise ValueError("Eurostat retail trade snapshot method differs")
    expected_format = {
        EurostatRetailTradeArtifactRole.SEARCH_INDEX: OfficialSourceFormat.ATOM,
        EurostatRetailTradeArtifactRole.CURRENT_DATASET: OfficialSourceFormat.JSON_STAT,
        EurostatRetailTradeArtifactRole.RELEASE_HTML: OfficialSourceFormat.HTML,
    ...
    if snapshot.request.source_format is not expected_format:
        raise ValueError(
            "Eurostat retail trade snapshot format differs from artifact role"
        )
    if snapshot.status_code != 200:
        raise ValueError("Eurostat retail trade snapshot status differs")
    if role is EurostatRetailTradeArtifactRole.SEARCH_INDEX:
        if (
            ...
            raise ValueError(
                "Eurostat retail trade search request identity differs"
            )
        try:
            root = safe_xml_fromstring(snapshot.content)
        except (DefusedXmlException, ParseError) as exc:
            raise ValueError(
                "Eurostat retail trade search artifact is invalid XML"
            ) from exc
        if root.tag != f"{{{_ATOM_NAMESPACE}}}feed":
            raise ValueError(
                "Eurostat retail trade search artifact is not Atom"
            )
    elif role is EurostatRetailTradeArtifactRole.CURRENT_DATASET:
        if (
            snapshot.request.page_number is not None
            or snapshot.request.uri != EUROSTAT_RETAIL_TRADE_DATASET_URI
        ):
            raise ValueError(
                "Eurostat retail trade dataset request identity differs"
            )
        if not snapshot.content.lstrip().startswith(b"{"):
            raise ValueError(
                "Eurostat retail trade dataset artifact is not JSON"
            )
    elif snapshot.request.page_number is not None:
        raise ValueError(
            "Eurostat retail trade release request has a page number"
        )
    elif role in {
        ...
        if b"<html" not in snapshot.content[:16_384].lower():
            raise ValueError("Eurostat retail trade HTML signature differs")
    elif not snapshot.content.startswith(b"%PDF-"):
        raise ValueError("Eurostat retail trade PDF signature differs")
    if role in {
        EurostatRetailTradeArtifactRole.RELEASE_DOCUMENT_HTML,
        ...
        raise ValueError("Eurostat retail trade document product code differs")
    return EurostatRetailTradeArtifactV1(
        role=role,
...
def _atom_text(entry: Element, tag: str) -> str:
    ...
    if node is None or node.text is None:
        raise ValueError(f"Eurostat retail trade Atom entry omits {tag}")
...
def _atom_link(entry: Element) -> str:
    ...
    if len(links) != 1 or "href" not in links[0].attrib:
        raise ValueError(
            "Eurostat retail trade Atom entry alternate link differs"
        )
    return _https_uri(links[0].attrib["href"], "source_uri")


def parse_eurostat_retail_trade_inventory(
    snapshots: Sequence[OfficialRawSnapshotV1],
    *,
    as_of_date: str,
) -> EurostatRetailTradeInventoryV1:
    """Parse and prove the complete bounded retail trade search selection."""
    as_of = _iso_date(as_of_date, "as_of_date")
    values = tuple(snapshots)
    if len(values) != EUROSTAT_RETAIL_TRADE_SEARCH_PAGE_COUNT:
        raise ValueError(
            "Eurostat retail trade inventory snapshot count differs"
        )
    if [item.request.page_number for item in values] != list(
        range(1, EUROSTAT_RETAIL_TRADE_SEARCH_PAGE_COUNT + 1)
    ):
        raise ValueError(
            "Eurostat retail trade search snapshots are not contiguous"
        )
    artifacts: list[EurostatRetailTradeArtifactV1] = []
    raw_entries: list[tuple[int, str, str, str, str, str]] = []
    for snapshot in values:
        artifact = _artifact_from_snapshot(
            snapshot, EurostatRetailTradeArtifactRole.SEARCH_INDEX
        )
        ...
        try:
            ...
            raise ValueError(
                "Eurostat retail trade search page is invalid XML"
            ) from exc
        entries = tuple(root.findall(f"{{{_ATOM_NAMESPACE}}}entry"))
        expected_count = (
            17
            if artifact.page_number == EUROSTAT_RETAIL_TRADE_SEARCH_PAGE_COUNT
            else 100
        )
        if len(entries) != expected_count:
            raise ValueError("Eurostat retail trade search page size differs")
    ...
    if len(raw_entries) != EUROSTAT_RETAIL_TRADE_SEARCH_RESULT_COUNT:
        raise ValueError("Eurostat retail trade search result count differs")
    ...
    for item in raw_entries:
        ...
        if previous is not None and previous[1:] != item[1:]:
            raise ValueError(
                "Eurostat retail trade search duplicate URI differs"
            )
        unique_by_uri.setdefault(item[2], item)
    if len(unique_by_uri) != EUROSTAT_RETAIL_TRADE_SEARCH_UNIQUE_URI_COUNT:
        raise ValueError(
            "Eurostat retail trade search unique URI count differs"
        )
    unique_entries = tuple(unique_by_uri.values())
    title_match_count = sum(
        re.search(r"\bretail trade\b", title, re.IGNORECASE) is not None
        for _, title, _, _, _, _ in unique_entries
    )
    prefiltered = [
        item for item in unique_entries if _inventory_prefilter(item[1])
    ]
    selected: list[EurostatRetailTradeInventoryEntryV1] = []
    exclusions: list[EurostatRetailTradeIndexExclusionV1] = []
    ...
    return EurostatRetailTradeInventoryV1(
...
def parse_eurostat_retail_trade_dataset(
    snapshot: OfficialRawSnapshotV1,
) -> EurostatRetailTradeDatasetV1:
    """Decode the exact current-revised retail trade JSON-stat cross-check."""
    artifact = _artifact_from_snapshot(
        snapshot, EurostatRetailTradeArtifactRole.CURRENT_DATASET
    )
    try:
        ...
        raise ValueError(
            "Eurostat retail trade dataset is invalid JSON"
        ) from exc
    ...
    if identifiers != expected_ids:
        raise ValueError("Eurostat retail trade dataset dimensions differ")
    sizes = tuple(_sequence(payload.get("size"), "size"))
    if any(
        isinstance(item, bool) or not isinstance(item, int) for item in sizes
    ):
        raise TypeError(
            "Eurostat retail trade dimension sizes must be integers"
        )
    ...
    if observed_codes != expected_codes:
        raise ValueError(
            "Eurostat retail trade dataset dimensions or codes differ"
        )
    ...
    if not time_codes or any(
        ...
        raise ValueError("Eurostat retail trade dataset time codes differ")
    if time_codes != _month_range(time_codes[0], time_codes[-1]):
        raise ValueError(
            "Eurostat retail trade dataset time codes are not contiguous"
        )
    expected_sizes = (1, 1, 1, 2, 2, len(_ECONOMIES), len(time_codes))
    if tuple(cast(Sequence[int], sizes)) != expected_sizes:
        raise ValueError("Eurostat retail trade dataset sizes differ")
    ...
    if set(values) != set(lexical_values):
        raise ValueError(
            "Eurostat retail trade lexical and numeric values differ"
        )
    if not set(statuses) <= set(values):
        raise ValueError("Eurostat retail trade status has no observation")
    observations: list[EurostatRetailTradeObservationV1] = []
    ...
    for raw_index, raw_value in values.items():
        try:
            ...
            raise ValueError(
                "Eurostat retail trade sparse index is invalid"
            ) from exc
        if not 0 <= flat_index < maximum:
            raise ValueError(
                "Eurostat retail trade sparse index is outside its cube"
            )
        ...
        measure = {
            (1, 0): EurostatRetailTradeMeasure.MONTH_OVER_MONTH,
            (0, 1): EurostatRetailTradeMeasure.YEAR_OVER_YEAR,
        }.get((s_adj_index, unit_index))
        if measure is None:
            raise ValueError(
                "Eurostat retail trade dataset populated an unsupported "
    ...
    return EurostatRetailTradeDatasetV1(
...
class EurostatRetailTradeMovement(str, Enum):
    ...
    @classmethod
    def from_value(
        cls, value: str | EurostatRetailTradeMovement
    ) -> EurostatRetailTradeMovement:
        try:
            return value if isinstance(value, cls) else cls(value)
        except ValueError as exc:
            raise ValueError(
                "unsupported Eurostat retail trade movement"
            ) from exc


def _headline_measure(reference_period: str) -> EurostatRetailTradeMeasure:
    ...
    return (
        ...
        else EurostatRetailTradeMeasure.YEAR_OVER_YEAR
    )


@dataclass(frozen=True, slots=True)
class EurostatRetailTradeReleaseValueV1:
    """Source-era euro-area retail trade rate in a release headline."""

    economy_code: str
    reference_period: str
    measure: EurostatRetailTradeMeasure
    movement: EurostatRetailTradeMovement
    value_lexical: str
    value: float
    evidence_id: str
    release_value_id: str = ""
    schema_version: str = EUROSTAT_RETAIL_TRADE_RELEASE_VALUE_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat retail trade release-value schema"
            )
        if self.economy_code != "EA":
            raise ValueError(
                "Eurostat retail trade release value must retain source-era EA scope"
            )
        reference = _month(self.reference_period, "reference_period")
        measure = EurostatRetailTradeMeasure.from_value(self.measure)
        if measure is not _headline_measure(reference):
            raise ValueError("Eurostat retail trade headline measure differs")
        movement = EurostatRetailTradeMovement.from_value(self.movement)
        lexical = _required_text(self.value_lexical, "value_lexical")
        if _NUMBER_RE.fullmatch(lexical) is None or lexical.startswith("-"):
            raise ValueError("Eurostat retail trade headline rate is invalid")
        magnitude = float(lexical)
        numeric = {
            EurostatRetailTradeMovement.DOWN: -magnitude,
            EurostatRetailTradeMovement.STABLE: 0.0,
        }.get(movement, magnitude)
        if self.value != numeric or not math.isfinite(self.value):
            raise ValueError(
                "Eurostat retail trade headline numeric value differs"
            )
        if not -100.0 <= self.value <= 100.0:
            raise ValueError(
                "Eurostat retail trade headline value is outside its bound"
            )
        evidence_id = _required_text(self.evidence_id, "evidence_id")
        if not evidence_id.startswith(
            "eurostat-retail-trade-inventory-entry:sha256:"
        ):
            raise ValueError("Eurostat retail trade headline evidence differs")
        object.__setattr__(self, "reference_period", reference)
        object.__setattr__(self, "measure", measure)
        ...
        expected = _stable_id(
            "eurostat-retail-trade-release-value",
            self.identity_payload(),
        )
        if self.release_value_id and self.release_value_id != expected:
            raise ValueError(
                "Eurostat retail trade release-value identity differs"
            )
    ...
    def from_dict(
        ...
        return cls(
            ...
            movement=EurostatRetailTradeMovement.from_value(
...
@dataclass(frozen=True, slots=True)
class EurostatRetailTradePublishedValueV1:
    ...
    schema_version: str = EUROSTAT_RETAIL_TRADE_PUBLISHED_VALUE_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "unsupported Eurostat retail trade published-value schema"
            )
        economy = _required_text(self.economy_code, "economy_code")
        if economy not in _RELEASE_ECONOMIES:
            raise ValueError(
                "Eurostat retail trade published-value economy differs"
            )
        reference = _month(self.reference_period, "reference_period")
        measure = EurostatRetailTradeMeasure.from_value(self.measure)
        lexical = _required_text(self.value_lexical, "value_lexical")
        if _SIGNED_NUMBER_RE.fullmatch(lexical) is None:
            raise ValueError(
                "Eurostat retail trade published-value lexical is invalid"
            )
        numeric = float(lexical)
        if numeric != self.value or not math.isfinite(numeric):
            raise ValueError(
                "Eurostat retail trade published-value numeric differs"
            )
        if not -100.0 <= numeric <= 100.0:
            raise ValueError(
                "Eurostat retail trade published value is outside its bound"
            )
        digest = _sha256(
        ...
        expected = _stable_id(
            "eurostat-retail-trade-published-value",
            self.identity_payload(),
        )
        if self.published_value_id and self.published_value_id != expected:
            raise ValueError(
                "Eurostat retail trade published-value identity differs"
            )
...
def _release_lag(release_date: str, reference_period: str) -> int:
    ...
    if not 20 <= lag <= 75:
        raise ValueError(
            "Eurostat retail trade release lag is outside the qualified window"
        )
    return lag
...
_HEADLINE_VALUE_RE = re.compile(
    ...
    r"(?P<value>\d+(?:\.\d+)?)\s*%|"
    r"(?:(?:remained|was)\s+)?(?P<stable>stable|unchanged)\b)",
    re.IGNORECASE,
)
...
def _headline_value(
    ...
    if match is None:
        raise ValueError("Eurostat retail trade release headline has no rate")
    movement_token = (
    ...
    movement = (
        ...
        else EurostatRetailTradeMovement.from_value(movement_token)
    )
    ...
    return EurostatRetailTradeReleaseValueV1(
        economy_code="EA",
...
def _parse_release_landing(
    ...
    try:
        ...
        raise ValueError(
            "Eurostat retail trade release landing is not UTF-8"
        ) from exc
    ...
    if titles != (entry.source_title,):
        raise ValueError("Eurostat retail trade Atom and landing titles differ")
    ...
    for date_match in _LANDING_DATE_RE.finditer(text):
        ...
        if month is None:
            raise ValueError(
                "Eurostat retail trade landing month is unsupported"
            )
        try:
            page_dates.add(
                date(
            ...
            raise ValueError(
                "Eurostat retail trade landing date is invalid"
            ) from exc
    ...
    if len(published_values) > 1:
        raise ValueError(
            "Eurostat retail trade release landing has no unique publication time"
        )
    ...
    if len(page_dates) != 1:
        raise ValueError(
            "Eurostat retail trade release landing has no unique release date"
        )
    ...
    if len(document_uris_by_identity) > 1:
        raise ValueError(
            "Eurostat retail trade landing exposes multiple English release documents"
        )
    document_uris = tuple(
        min(
            variants,
...
@lru_cache(maxsize=MAX_EUROSTAT_RETAIL_TRADE_RELEASES)
def _retail_pdf_page_texts(content: bytes) -> tuple[str, ...]:
    ...
    try:
        ...
        if len(failed_pages) > 1 or not any(item.strip() for item in page_text):
            raise ValueError(
                "Eurostat retail trade release PDF has too many unreadable pages"
            )
        return tuple(page_text)
    except PdfReadError as exc:
        raise ValueError(
            "Eurostat retail trade release PDF text extraction failed"
        ) from exc
...
def _release_source_text(snapshot: OfficialRawSnapshotV1) -> str:
    if snapshot.request.source_format is OfficialSourceFormat.HTML:
        ...
        text = " ".join(_retail_pdf_page_texts(snapshot.content))
    else:
        raise ValueError(
            "Eurostat retail trade release value source format differs"
        )
    normalized = _SPACE_RE.sub(" ", text.replace("\u2212", "-")).strip()
    if not normalized:
        raise ValueError(
            "Eurostat retail trade release value source has no text"
        )
    return normalized
...
def _reference_period_from_publication(
    ...
    inferred = EUROSTAT_RETAIL_TRADE_REFERENCE_PERIOD_INFERENCES.get(
        entry.product_code
    )
    if inferred is not None:
        return inferred, _release_lag(entry.release_date, inferred)
    raise ValueError(
        "Eurostat retail trade publication omits a qualified reference month for "
...
def _composition_from_publication(
    ...
    if labels and expected not in labels:
        raise ValueError(
            "Eurostat retail trade source-stated EA composition differs: "
            f"expected={expected}, observed={tuple(sorted(labels))}"
        )
    return expected, expected in labels


class _RetailTradeTableParser(HTMLParser):
    ...
    def __init__(self) -> None:
        ...
        self.table_measures: list[EurostatRetailTradeMeasure | None] = []
        self._table: list[tuple[str, ...]] | None = None
        self._table_context = ""
        self._active_measure: EurostatRetailTradeMeasure | None = None
        ...
        if self._cell is not None:
            ...
            if value:
                ...
                if (
                    ...
                    "retail trade" in normalized
                    and ("index" in normalized or "indices" in normalized)
                    and "%" not in normalized
                    and "change" not in normalized
                ):
...
_RETAIL_PERIOD_CELL_RE = re.compile(
    r"^(?:(?P<name>[A-Za-z]{3,9})[-/ ](?P<name_year>\d{2,4})|"
    r"(?P<number>0?[1-9]|1[0-2])/(?P<number_year>\d{2,4})|"
    r"(?P<year>20\d{2})[-/](?P<year_month>0?[1-9]|1[0-2]))$"
)
_RETAIL_VALUE_CELL_RE = re.compile(r"^[+\-\u2212]?(?:\d+(?:\.\d+)?|\.\d+)")
_RETAIL_INDEX_MARKER_RE = re.compile(
    r"(?:\b(?:index|indices)\b|\b\d{4}\s*=\s*100\b|"
    r"\bbase\s+year\s+(?:19|20)\d{2}\b)",
    re.IGNORECASE,
)
_RETAIL_MONTHLY_MARKERS: Final = (
    "previous month",
    "monthly comparison",
    "month-on-month",
    "month on month",
)
_RETAIL_ANNUAL_MARKERS: Final = (
...
_RETAIL_PERIOD_TOKEN_RE = re.compile(
    r"(?<![A-Za-z0-9])(?:"
...
_RETAIL_PDF_NUMBER_RE = re.compile(
    r"(?<![A-Za-z0-9.])[+\-−]?(?:\d+(?:\.\d+)?|\.\d+)(?![A-Za-z0-9.])|:"
)


def _retail_period_cell(value: str) -> str | None:
    token = _SPACE_RE.sub(" ", value).strip(" *\u00a0")
    match = _RETAIL_PERIOD_CELL_RE.fullmatch(token)
    if match is None:
        return None
    if match.group("name"):
        month = _RETAIL_MONTH_NAMES.get(match.group("name")[:3].casefold())
    ...
    year = (
        numeric_year + (1900 if numeric_year >= 90 else 2000)
        if len(raw_year) == 2
        else numeric_year
    )
...
def _retail_table_measure(
    rows: Sequence[Sequence[str]],
) -> EurostatRetailTradeMeasure | None:
    text = " ".join(cell.casefold() for row in rows[:4] for cell in row if cell)
    if any(marker in text for marker in _RETAIL_ANNUAL_MARKERS):
        return EurostatRetailTradeMeasure.YEAR_OVER_YEAR
    if any(marker in text for marker in _RETAIL_MONTHLY_MARKERS):
        return EurostatRetailTradeMeasure.MONTH_OVER_MONTH
    return None


def _retail_context_measure(value: str) -> EurostatRetailTradeMeasure | None:
    normalized = value.casefold()
    candidates = (
        (
            max(
                (normalized.rfind(marker) for marker in _RETAIL_MONTHLY_MARKERS)
            ),
            EurostatRetailTradeMeasure.MONTH_OVER_MONTH,
        ),
        (
            max(normalized.rfind(marker) for marker in _RETAIL_ANNUAL_MARKERS),
            EurostatRetailTradeMeasure.YEAR_OVER_YEAR,
        ),
    )
    position, measure = max(candidates, key=lambda item: item[0])
    return measure if position >= 0 else None


def _retail_economy_cell(
...
def _retail_value_cell(value: str) -> str | None:
    token = _SPACE_RE.sub(" ", value).strip()
    if token in {"", ":", "-", ".."}:
        return None
    match = _RETAIL_VALUE_CELL_RE.match(token)
    if match is None:
        return None
    return match.group(0).replace("\u2212", "-")


def _retail_html_table_values(
    snapshot: OfficialRawSnapshotV1,
    artifact: EurostatRetailTradeArtifactV1,
    reference_period: str,
) -> tuple[EurostatRetailTradePublishedValueV1, ...]:
    ...
    try:
        ...
        source = snapshot.content.decode("cp1252")
    parser = _RetailTradeTableParser()
    parser.feed(source)
    parser.close()
    expected_ea_composition = _ea_composition_for_period(reference_period)
    values: dict[
        tuple[str, str, EurostatRetailTradeMeasure],
        EurostatRetailTradePublishedValueV1,
    ] = {}
    for table_number, (context, declared_measure, rows) in enumerate(
        ...
        table_descriptor = f"{context} {table_header}".casefold()
        if (
            "retail trade" in table_descriptor
            and _RETAIL_INDEX_MARKER_RE.search(table_descriptor) is not None
            and _retail_table_measure(rows) is None
        ):
            continue
        measure = (
            _retail_table_measure(rows)
            or _retail_context_measure(context)
            or declared_measure
        )
        if measure is None:
            continue
        ...
        for row_index, row in enumerate(rows):
            candidates = {
                ...
                if (period := _retail_period_cell(cell)) is not None
            }
        ...
        for row in rows[header_index + 1 :]:
            economy = next(
                (
                    ...
                    for cell in row[:3]
                    if (
                        candidate := _retail_economy_cell(
                            cell,
            ...
            for column, period in period_columns.items():
                if (
                    ...
                    continue
                lexical = _retail_value_cell(row[column])
                if lexical is None:
                    continue
                item = EurostatRetailTradePublishedValueV1(
                    ...
                    evidence_locator=(
                        f"retail-trade HTML table {table_number}; "
                ...
                if previous is not None and previous.value != item.value:
                    raise ValueError(
                        "Eurostat retail trade HTML tables conflict for "
...
def _retail_pdf_page_values(
    ...
    measure = _retail_table_measure((lines[:8],))
    if measure is None:
        return ()
    ...
    for line in lines:
        periods = tuple(
            period
            for match in _RETAIL_PERIOD_TOKEN_RE.finditer(line)
            if (period := _retail_period_cell(match.group(0))) is not None
        )
        correction = EUROSTAT_RETAIL_TRADE_PERIOD_HEADER_CORRECTIONS.get(
        ...
        if not period_columns:
            continue
        row_match = _RETAIL_PDF_ROW_RE.match(line)
        if row_match is not None:
            economy = _retail_economy_cell(
            ...
            r"^(?:Total(?: retail trade)?|Retail trade(?: total)?)\d*\b",
            line,
            re.IGNORECASE,
        ):
        ...
        pending_economy = None
        body = re.sub(
            r"^(?:Total(?: retail trade)?|Retail trade(?: total)?)\d*\s*",
            "",
            body,
            flags=re.IGNORECASE,
        )
        tokens = tuple(
            ...
            for match in _RETAIL_PDF_NUMBER_RE.finditer(body)
        )
        ...
        for period, lexical in zip(period_columns, tokens, strict=True):
            ...
            item = EurostatRetailTradePublishedValueV1(
                ...
                evidence_artifact_sha256=artifact.content_sha256,
                evidence_locator=(
                    f"retail-trade PDF page {page_number}; "
                    f"{economy} row; {period}; {measure.value}"
                ),
            )
            ...
            if previous is not None and previous.value != item.value:
                raise ValueError(
                    "Eurostat retail trade PDF tables conflict for "
                    f"{artifact.product_code}: {economy} {period} {measure.value}"
                )
...
def _retail_pdf_table_values(
    ...
    for page_number, page_text in enumerate(
        _retail_pdf_page_texts(snapshot.content), start=1
    ):
        if page_text.strip():
            readable_page_count += 1
        for item in _retail_pdf_page_values(
            ...
            if previous is not None and previous.value != item.value:
                raise ValueError(
                    "Eurostat retail trade PDF pages conflict for "
    ...
    if not readable_page_count:
        raise ValueError(
            "Eurostat retail trade release PDF table has no readable text"
        )
    return tuple(
        sorted(
            values.values(),
...
def _retail_table_published_values(
    snapshot: OfficialRawSnapshotV1,
    artifact: EurostatRetailTradeArtifactV1,
    reference_period: str,
) -> tuple[EurostatRetailTradePublishedValueV1, ...]:
    values = _retail_html_table_values(snapshot, artifact, reference_period)
    if values:
        return values
    return _retail_pdf_table_values(snapshot, artifact, reference_period)


def _published_values(
    landing_snapshot: OfficialRawSnapshotV1,
    landing_artifact: EurostatRetailTradeArtifactV1,
    document_snapshot: OfficialRawSnapshotV1 | None,
    document_artifact: EurostatRetailTradeArtifactV1 | None,
    reference_period: str,
    headline: EurostatRetailTradeReleaseValueV1,
) -> tuple[EurostatRetailTradePublishedValueV1, ...]:
    parsed: tuple[EurostatRetailTradePublishedValueV1, ...] = ()
    for snapshot, artifact in (
        ...
        if snapshot is None or artifact is None:
            continue
        parsed = _retail_table_published_values(
    ...
    headline_match = next(
        (
            ...
            and item.measure is headline.measure
    ...
    if headline_match is None:
        values.append(
            EurostatRetailTradePublishedValueV1(
                ...
                value_lexical=(
                    ...
                    if headline.movement is EurostatRetailTradeMovement.DOWN
        ...
        raise ValueError(
            "Eurostat retail trade headline and publication table differ for "
...
@dataclass(frozen=True, slots=True)
class EurostatRetailTradeReleaseV1:
    """One source-dated monthly retail trade release."""

    inventory_entry: EurostatRetailTradeInventoryEntryV1
    reference_period: str
    ...
    artifact: EurostatRetailTradeArtifactV1
    document_artifact: EurostatRetailTradeArtifactV1 | None
    headline_value: EurostatRetailTradeReleaseValueV1
    published_values: tuple[EurostatRetailTradePublishedValueV1, ...]
    release_id: str = ""
    schema_version: str = EUROSTAT_RETAIL_TRADE_RELEASE_SCHEMA_VERSION

    def __post_init__(self) -> None:
        if self.schema_version != EUROSTAT_RETAIL_TRADE_RELEASE_SCHEMA_VERSION:
            raise ValueError("unsupported Eurostat retail trade release schema")
        reference = _month(self.reference_period, "reference_period")
        if not isinstance(self.reference_period_source_stated, bool):
            raise TypeError("reference_period_source_stated must be boolean")
        inferred_reference = (
            EUROSTAT_RETAIL_TRADE_REFERENCE_PERIOD_INFERENCES.get(
        ...
        if (
            ...
            raise ValueError(
                "Eurostat retail trade reference-period evidence differs"
            )
        ...
        if self.days_after_period_end != expected_lag:
            raise ValueError(
                "Eurostat retail trade release period or lag differs"
            )
        ...
        if composition != _ea_composition_for_period(reference):
            raise ValueError("Eurostat retail trade EA composition differs")
        ...
        expected_exception = (
            EUROSTAT_RETAIL_TRADE_PAGE_DATE_OFFSET_EXCEPTIONS.get(
                self.inventory_entry.product_code
            )
        )
        if self.page_release_date_offset_days != offset or (
            ...
            raise ValueError(
                "Eurostat retail trade landing date offset differs for "
        ...
        if published_at is not None:
            ...
            if published_date.isoformat() != page_date:
                raise ValueError(
                    "Eurostat retail trade publication timestamp date differs"
                )
        links = tuple(
        ...
        if links != tuple(sorted(set(links))):
            raise ValueError(
                "Eurostat retail trade release document links are not canonical"
            )
        if len(links) > 1:
            raise ValueError(
                "Eurostat retail trade release has multiple English documents"
            )
        if any(
            ...
            raise ValueError(
                "Eurostat retail trade release document product code differs"
            )
        if (
            self.artifact.role
            is not EurostatRetailTradeArtifactRole.RELEASE_HTML
        ):
            raise ValueError(
                "Eurostat retail trade release artifact role differs"
            )
        if (
            ...
            raise ValueError(
                "Eurostat retail trade release artifact product code differs"
            )
        document = self.document_artifact
        if (document is None) != (not links):
            raise ValueError(
                "Eurostat retail trade release document artifact differs"
            )
        if document is not None and (
            ...
            raise ValueError(
                "Eurostat retail trade release document identity differs"
            )
        ...
        if (
            ...
            raise ValueError(
                "Eurostat retail trade release headline occurrence differs"
            )
        published_values = tuple(self.published_values)
        if not (
            1
            <= len(published_values)
            <= MAX_EUROSTAT_RETAIL_TRADE_PUBLISHED_VALUES_PER_RELEASE
        ):
            raise ValueError(
                "Eurostat retail trade published-value count differs"
            )
        keys = [item.key for item in published_values]
        if len(keys) != len(set(keys)):
            raise ValueError(
                "Eurostat retail trade release repeats a published value"
            )
        if list(published_values) != sorted(
            ...
            raise ValueError(
                "Eurostat retail trade published values are not canonical"
            )
        ...
        if any(
            ...
            raise ValueError(
                "Eurostat retail trade published-value evidence differs"
            )
        ...
        if (
            ...
            raise ValueError(
                "Eurostat retail trade headline published value differs"
            )
        object.__setattr__(self, "reference_period", reference)
        ...
        expected = _stable_id(
            "eurostat-retail-trade-release", self.identity_payload()
        )
        if self.release_id and self.release_id != expected:
            raise ValueError("Eurostat retail trade release identity differs")
    ...
    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> EurostatRetailTradeReleaseV1:
        return cls(
            inventory_entry=EurostatRetailTradeInventoryEntryV1.from_dict(
            ...
            headline_value=EurostatRetailTradeReleaseValueV1.from_dict(
                _mapping(data.get("headline_value"), "headline_value")
            ),
            published_values=tuple(
                EurostatRetailTradePublishedValueV1.from_dict(
...
def _published_revision_counts(
    releases: Sequence[EurostatRetailTradeReleaseV1],
) -> tuple[int, int]:
...
def _current_dataset_comparison_counts(
    releases: Sequence[EurostatRetailTradeReleaseV1],
    dataset: EurostatRetailTradeDatasetV1,
) -> tuple[int, int]:
    current = {
...
@dataclass(frozen=True, slots=True)
class EurostatRetailTradeArchiveManifestV1:
    """Compact, replayable receipt for the official retail trade corpus."""

    as_of_date: str
    registry_id: str
    source_id: str
    inventory: EurostatRetailTradeInventoryV1
    current_dataset: EurostatRetailTradeDatasetV1
    releases: tuple[EurostatRetailTradeReleaseV1, ...]
    ...
    manifest_id: str = ""
    schema_version: str = EUROSTAT_RETAIL_TRADE_MANIFEST_SCHEMA_VERSION

    def __post_init__(self) -> None:
        if self.schema_version != EUROSTAT_RETAIL_TRADE_MANIFEST_SCHEMA_VERSION:
            raise ValueError(
                "unsupported Eurostat retail trade manifest schema"
            )
        as_of = _iso_date(self.as_of_date, "as_of_date")
        if as_of != EUROSTAT_RETAIL_TRADE_AS_OF_DATE:
            raise ValueError("Eurostat retail trade archive boundary differs")
        ...
        if not registry_id.startswith("official-source-registry:sha256:"):
            raise ValueError(
                "Eurostat retail trade registry identity is invalid"
            )
        if not source_id.startswith("official-source:sha256:"):
            raise ValueError("Eurostat retail trade source identity is invalid")
        releases = tuple(self.releases)
        if len(releases) != EUROSTAT_RETAIL_TRADE_RELEASE_COUNT:
            raise ValueError(
                "Eurostat retail trade manifest release count differs"
            )
        if list(releases) != sorted(
            ...
            raise ValueError("Eurostat retail trade releases are not canonical")
        if [item.inventory_entry for item in releases] != list(
            self.inventory.entries
        ):
            raise ValueError(
                "Eurostat retail trade release and inventory entries differ"
            )
        if any(
            item.headline_value
            ...
            raise ValueError(
                "Eurostat retail trade release headline evidence differs"
            )
        occurrences = [item.occurrence_key for item in releases]
        if len(occurrences) != len(set(occurrences)):
            raise ValueError(
                "Eurostat retail trade releases repeat an occurrence"
            )
        periods = [item.reference_period for item in releases]
        if len(periods) != len(set(periods)):
            raise ValueError("Eurostat retail trade releases repeat a month")
        if (
            min(periods) != EUROSTAT_RETAIL_TRADE_FIRST_REFERENCE_PERIOD
            or max(periods) != EUROSTAT_RETAIL_TRADE_LATEST_REFERENCE_PERIOD
        ):
            raise ValueError("Eurostat retail trade reference range differs")
        expected_release_periods = tuple(
            ...
            if item not in EUROSTAT_RETAIL_TRADE_INTERNAL_GAP_REFERENCE_PERIODS
        )
        if tuple(periods) != expected_release_periods:
            ...
            raise ValueError(
                "Eurostat retail trade reference-month lineage differs: "
                f"actual gaps={actual_gaps!r}"
            )
        if any(item.inventory_entry.release_date > as_of for item in releases):
            raise ValueError(
                "Eurostat retail trade release exceeds archive boundary"
            )
        if self.current_dataset.updated_at[:10] > as_of:
            raise ValueError(
                "Eurostat retail trade dataset exceeds archive boundary"
            )
        expected_gaps = (
            *_month_range(
                self.current_dataset.time_start,
                _month_shift(EUROSTAT_RETAIL_TRADE_FIRST_REFERENCE_PERIOD, -1),
            ),
        ...
        if gaps != expected_gaps:
            raise ValueError(
                "Eurostat retail trade searchable-release gap inventory differs"
            )
        for name, maximum in (
            (
                "reference_period_inferred_count",
                MAX_EUROSTAT_RETAIL_TRADE_RELEASES,
            ),
            (
        ...
        if self.reference_period_inferred_count != expected_inferred_references:
            raise ValueError(
                "Eurostat retail trade inferred-reference count differs"
            )
        if self.composition_source_stated_count != sum(
            item.composition_source_stated for item in releases
        ):
            raise ValueError("Eurostat retail trade composition count differs")
        ...
        if self.page_date_offset_count != expected_offsets:
            raise ValueError(
                "Eurostat retail trade page-date offset count differs"
            )
        ...
        if self.exact_publication_time_count != expected_exact_times:
            raise ValueError(
                "Eurostat retail trade exact publication-time count differs"
            )
        ...
        if len(document_artifacts) != EUROSTAT_RETAIL_TRADE_DOCUMENT_COUNT:
            raise ValueError(
                "Eurostat retail trade English document count differs"
            )
        if self.release_document_count != len(document_artifacts):
            raise ValueError(
                "Eurostat retail trade release-document count differs"
            )
        ...
        if revision_counts != (
            ...
            raise ValueError("Eurostat retail trade revision counts differ")
        ...
        if dataset_counts != (
            ...
            raise ValueError("Eurostat retail trade dataset comparison differs")
        artifacts = (
        ...
        counts = (
            _bounded_int(
                ...
                "raw_artifact_count",
                MAX_EUROSTAT_RETAIL_TRADE_ARTIFACTS,
            ),
            _bounded_int(
        ...
        if counts != (
            ...
            raise ValueError("Eurostat retail trade artifact totals differ")
        ...
        expected = _stable_id(
            "eurostat-retail-trade-archive-manifest",
            self.identity_payload(),
        )
        if self.manifest_id and self.manifest_id != expected:
            raise ValueError("Eurostat retail trade manifest identity differs")
    ...
    def from_dict(
        ...
        return cls(
            ...
            inventory=EurostatRetailTradeInventoryV1.from_dict(
                _mapping(data.get("inventory"), "inventory")
            ),
            current_dataset=EurostatRetailTradeDatasetV1.from_dict(
                _mapping(data.get("current_dataset"), "current_dataset")
            ),
            releases=tuple(
                EurostatRetailTradeReleaseV1.from_dict(
    ...
    @classmethod
    def from_json(cls, value: str) -> EurostatRetailTradeArchiveManifestV1:
        try:
            data = json.loads(value)
        except json.JSONDecodeError as exc:
            raise ValueError(
                "Eurostat retail trade manifest is invalid JSON"
            ) from exc
        return cls.from_dict(_mapping(data, "manifest"))


def build_eurostat_retail_trade_archive_manifest(
    registry: OfficialSourceRegistryV1,
    ...
    *,
    ...
    """Build the retail trade receipt from exact retained Eurostat bytes."""
    source = registry.source(EUROSTAT_RETAIL_TRADE_SOURCE_KEY)
    if (
        source.parser_id != EUROSTAT_RETAIL_TRADE_PARSER_ID
        or source.parser_version != EUROSTAT_RETAIL_TRADE_PARSER_VERSION
    ):
        raise ValueError(
            "Eurostat retail trade registry parser identity differs"
        )
    ...
    if any(
        ...
        raise ValueError(
            "Eurostat retail trade snapshot source identity differs"
        )
    inventory = parse_eurostat_retail_trade_inventory(
        search_values, as_of_date=as_of_date
    )
    dataset = parse_eurostat_retail_trade_dataset(dataset_snapshot)
    snapshots = release_values
    if len(snapshots) != EUROSTAT_RETAIL_TRADE_RELEASE_COUNT:
        raise ValueError("Eurostat retail trade release snapshot count differs")
    by_code: dict[str, OfficialRawSnapshotV1] = {}
    for snapshot in snapshots:
        code = _product_code(snapshot.request.uri)
        if code in by_code:
            raise ValueError(
                "Eurostat retail trade release corpus repeats a product code"
            )
        by_code[code] = snapshot
    if set(by_code) != {item.product_code for item in inventory.entries}:
        raise ValueError("Eurostat retail trade release corpus is incomplete")
    if len(document_values) != EUROSTAT_RETAIL_TRADE_DOCUMENT_COUNT:
        raise ValueError(
            "Eurostat retail trade document snapshot count differs"
        )
    ...
    for snapshot in document_values:
        if snapshot.request.uri in documents_by_uri:
            raise ValueError(
                "Eurostat retail trade document corpus repeats a URI"
            )
    ...
    parsed: list[
        tuple[
            EurostatRetailTradeInventoryEntryV1,
            EurostatRetailTradeArtifactV1,
            EurostatRetailTradeArtifactV1 | None,
            str,
            str | None,
            str,
            int,
    ...
    for entry in inventory.entries:
        ...
        if snapshot.request.uri != entry.source_uri:
            raise ValueError(
                "Eurostat retail trade release request identity differs"
            )
        artifact = _artifact_from_snapshot(
            snapshot,
            EurostatRetailTradeArtifactRole.RELEASE_HTML,
        ...
        document_artifact: EurostatRetailTradeArtifactV1 | None = None
        ...
        if document_uris:
            ...
            try:
                ...
                raise ValueError(
                    "Eurostat retail trade document corpus is incomplete"
                ) from exc
            ...
            if document_role is None:
                raise ValueError(
                    "Eurostat retail trade document format differs"
                )
    ...
    if used_document_uris != set(documents_by_uri):
        raise ValueError(
            "Eurostat retail trade document corpus has unexpected artifacts"
        )
    releases: list[EurostatRetailTradeReleaseV1] = []
    for (
        entry,
        artifact,
        document_artifact,
        ...
        if lag != expected_lag:
            raise ValueError(
                "Eurostat retail trade release lag changed during parsing"
            )
        ...
        releases.append(
            EurostatRetailTradeReleaseV1(
                ...
                reference_period_source_stated=(
                    entry.product_code
                    not in EUROSTAT_RETAIL_TRADE_REFERENCE_PERIOD_INFERENCES
                ),
    ...
    return EurostatRetailTradeArchiveManifestV1(
...
def replay_eurostat_retail_trade_archive(
    registry: OfficialSourceRegistryV1,
    search_snapshots: Sequence[OfficialRawSnapshotV1],
    dataset_snapshot: OfficialRawSnapshotV1,
    release_snapshots: Sequence[OfficialRawSnapshotV1],
    document_snapshots: Sequence[OfficialRawSnapshotV1],
    expected_manifest: EurostatRetailTradeArchiveManifestV1,
) -> EurostatRetailTradeArchiveManifestV1:
    """Rebuild the retail trade receipt and require byte-for-byte identity."""
    rebuilt = build_eurostat_retail_trade_archive_manifest(
        registry,
        search_snapshots,
        dataset_snapshot,
        release_snapshots,
    ...
    if rebuilt.to_json() != expected_manifest.to_json():
        raise ValueError(
            "Eurostat retail trade replay differs from packaged manifest"
        )
    return rebuilt


def packaged_eurostat_retail_trade_manifest_path() -> Path:
    """Return the installed retail trade archive-manifest path."""
    return (
        Path(__file__).resolve().parent
        / "assets"
        / "eurostat_retail_trade_archive_v1.json"
    )


def load_packaged_eurostat_retail_trade_archive_manifest() -> (
    EurostatRetailTradeArchiveManifestV1
):
    """Load and validate the installed retail trade archive manifest."""
    path = packaged_eurostat_retail_trade_manifest_path()
    if path.stat().st_size > 16 * 1024 * 1024:
        raise ValueError(
            "packaged Eurostat retail trade manifest exceeds size bound"
        )
    manifest = EurostatRetailTradeArchiveManifestV1.from_json(
    ...
    if (
        ...
        raise ValueError(
            "packaged Eurostat retail trade registry binding differs"
        )
    return manifest
