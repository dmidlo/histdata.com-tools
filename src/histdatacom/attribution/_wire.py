...
def plain(value: object) -> object:
    ...
    def visit(item: object, depth: int) -> object:
        ...
        if nodes > MAX_NODES or depth > MAX_DEPTH:
            raise ValueError("attribution expanded node/depth bound")
        ...
        if type(item) is dict:
            if len(item) > MAX_ITEMS:
                raise ValueError("attribution collection bound")
            size += 2 + 2 * len(item)
            output: dict[str, object] = {}
            for key, child in item.items():
                if type(key) is not str or len(key) > 256:
                    raise ValueError("invalid attribution object key")
                visit(key, depth + 1)
            ...
            if type(item) is str:
                if len(item) > MAX_TEXT:
                    raise ValueError("attribution string bound")
            elif type(item) is int:
                if not -(2**63) <= item < 2**63:
                    raise ValueError("attribution integer bound")
            elif type(item) is float:
                if not math.isfinite(item):
                    raise ValueError("attribution non-finite number")
            elif item is not None and type(item) is not bool:
                raise ValueError("unsupported attribution scalar")
            try:
                size += len(
                    json.dumps(item, ensure_ascii=True, allow_nan=False)
                )
            except (ValueError, TypeError) as exc:
                raise ValueError("invalid attribution scalar") from exc
            result = item
        if size > MAX_BYTES:
            raise ValueError("attribution byte bound")
        return result
...
def load_json(text: str) -> object:
    if type(text) is not str or len(text) > MAX_BYTES:
        raise ValueError("attribution JSON byte bound")
    ...
    try:
        ...
        if canonical_json(result) != text:
            raise ValueError("noncanonical attribution JSON")
    except (RecursionError, TypeError, OverflowError) as exc:
        raise ValueError("invalid attribution JSON") from exc
    return result
...
def identity(kind: str, payload: object) -> str:
    return f"attribution-{kind}:sha256:" + sha256(canonical_json(payload))


def text(value: str) -> None:
    if not value or value != value.strip() or any(ord(c) < 32 for c in value):
        raise ValueError("invalid attribution text")
...
def identifier(value: str, kind: str | None = None) -> None:
    pattern = r"attribution-[a-z-]+:sha256:[0-9a-f]{64}"
    if re.fullmatch(pattern, value) is None or (
        kind is not None and not value.startswith(f"attribution-{kind}:sha256:")
    ):
        raise ValueError("invalid attribution artifact identity")


def ordered(values: tuple[str, ...], *, nonempty: bool = False) -> None:
    if (nonempty and not values) or values != tuple(sorted(set(values))):
        raise ValueError("attribution values must be sorted unique")
    for value in values:
        text(value)
...
def _decode(hint: Any, value: object) -> Any:
    ...
    if origin in (Union, UnionType):
        for alternative in args:
            try:
                ...
                pass
        raise ValueError("invalid attribution union")
    if origin is tuple:
        if type(value) not in (tuple, list):
            raise ValueError("expected attribution tuple")
    ...
    if isinstance(hint, type) and issubclass(hint, Record):
        ...
        if type(value) is not dict:
            raise ValueError("expected attribution record")
        return hint.from_payload(cast(dict[str, object], value))
    if isinstance(hint, type) and issubclass(hint, Enum):
        if type(value) is hint:
            return value
        if type(value) is not str:
            raise ValueError("expected attribution enum")
        return hint(value)
    if type(value) is not hint:
        raise ValueError("invalid attribution scalar type")
    return value
...
class Record:
    ...
    def from_payload(
        ...
        if type(payload) is not dict or set(payload) != {
            ...
            raise ValueError("unknown or missing attribution field")
        return cls(**payload)
...
class Artifact(Record):
    ...
    def schema_version(cls) -> str:
        return f"histdatacom.attribution-{cls.KIND}.v1"
    ...
    def from_json(cls: type[_T], text: str) -> _T:  # noqa: PYI019
        ...
        if type(value) is not dict:
            raise ValueError("expected attribution envelope")
