...
from histdatacom.orchestration.client import submit_run_request_and_observe
from histdatacom.orchestration.control import JobLifecycle
from histdatacom.orchestration.live_smoke import (
    DEFAULT_LIVE_ORCHESTRATION_SMOKE_COMPLETION_TIMEOUT,
    DEFAULT_LIVE_ORCHESTRATION_SMOKE_LANES,
    DEFAULT_LIVE_ORCHESTRATION_SMOKE_STARTUP_TIMEOUT,
    DEFAULT_LIVE_ORCHESTRATION_SMOKE_STOP_TIMEOUT,
    TEMPORAL_EXECUTABLE_ENV,
    _stop_live_orchestration,
...
from histdatacom.orchestration.workflows import (
...
def accepted_performance_envelope() -> dict[str, JSONValue]:
    ...
    return {
        ...
        "orchestration_overhead": (
            "Live orchestration single-item requests include fixed Temporal startup "
...
def run_live_orchestration_throughput_benchmark(
    *,
    workspace: Path | str,
    ...
    try:
        ...
        runtime_results = {
            ...
            for result in (
                _benchmark_orchestration_scenario(
                    scenario,
...
def _benchmark_orchestration_scenario(
    scenario: ThroughputBenchmarkScenario,
    *,
    ...
    def run() -> None:
        captured["cpu_before"] = _orchestration_process_cpu_snapshot(supervisor)
        ...
        _validate_orchestration_job_result(result)
        captured["cpu_after"] = _orchestration_process_cpu_snapshot(supervisor)
        captured["result"] = result
        captured["payload"] = _orchestration_payload(result)
    ...
    orchestration_cpu_seconds = _cpu_delta_seconds(
    ...
    return RuntimeBenchmarkResult(
        ...
        metadata={
            ...
            "runtime_process_cpu_seconds": orchestration_cpu_seconds,
            "runtime_process_cpu_utilization_ratio": (
                orchestration_cpu_seconds / measurement.elapsed_seconds
...
def _validate_orchestration_job_result(result: Any) -> None:
    snapshot = getattr(result, "snapshot", None)
    if snapshot is not None and snapshot.lifecycle != JobLifecycle.SUCCEEDED:
        raise RuntimeError(
            f"orchestration benchmark job did not succeed: {snapshot.lifecycle}"
        )


def _orchestration_payload(result: Any) -> Mapping[str, Any]:
...
def _orchestration_process_cpu_snapshot(
    supervisor: OrchestrationSupervisor,
) -> dict[str, float]:
    try:
